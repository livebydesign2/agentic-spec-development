...
module.exports = {
  ...
  appIcon: team === 'frontend' ? '🎨' : team === 'backend' ? '⚙️' : team === 'devops' ? '🔧' : '📋',
...
function validateConfig(config) {
  ...
  // Array validations
  if (!Array.isArray(config.supportedTypes) || config.supportedTypes.length === 0) {
    throw new Error('supportedTypes must be a non-empty array');
  }
  
  if (!Array.isArray(config.statusFolders) || config.statusFolders.length === 0) {
    throw new Error('statusFolders must be a non-empty array');
  }
  
  // Priority validation
  if (config.defaultPriority && !config.priorities.includes(config.defaultPriority)) {
    throw new Error('defaultPriority must be one of the defined priorities');
  }
  
  // Status validation
  if (config.defaultStatus && !config.statusFolders.includes(config.defaultStatus)) {
