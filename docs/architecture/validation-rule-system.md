...
class ValidationRuleRegistry {
  ...
  getRuleMetadata(name) {
    return this.ruleMetadata.get(name);
  }
  getCategories() {
  ...
  async executeRules(data, rules, context, options = {}) {
    ...
    try {
      ...
      if (options.parallel && options.parallel !== false) {
        results.push(
          ...(await this.executeRulesParallel(orderedRules, data, context))
        );
      } else {
        results.push(
          ...(await this.executeRulesSequential(orderedRules, data, context))
        );
      }
...
class ValidationRule {
  ...
  async autoFix(data, error, context) {
    ...
    throw new Error(
      `Rule ${this.name} marked as auto-fixable but autoFix method not implemented`
    );
  }
...
class IDFormatRule extends ValidationRule {
  ...
  async validate(spec, context) {
    ...
    const duplicates = allSpecs.filter(
      (s) => s.id === spec.id && s.filePath !== spec.filePath
    );
  ...
  hasCircularDependency(task, dependency, allTasks, visited = new Set()) {
    ...
    if (dependency.depends_on) {
      for (const depId of dependency.depends_on) {
        ...
        if (
          nextDep &&
          this.hasCircularDependency(task, nextDep, allTasks, new Set(visited))
        ) {
