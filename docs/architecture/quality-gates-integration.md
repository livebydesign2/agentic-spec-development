...
class QualityGateManager {
  ...
  async enforceAssignmentGate(taskId, agentType, options = {}) {
    ...
    try {
      ...
      const gateResults = await this.executeGateRules(
        gateRules, 
        {
  ...
  async enforceTransitionGate(specId, fromStatus, toStatus, options = {}) {
    ...
    try {
      ...
      const context = await this.buildTransitionContext(specId, fromStatus, toStatus);
      ...
      if (ruleResult.success) {
        ...
        if (ruleResult.severity === 'error') {
          ...
          if (!evaluation.reason) {
            ...
            evaluation.reason = primaryError?.message || 
  ...
  async buildAssignmentContext(taskId, agentType) {
    try {
      ...
      const spec = await this.validationManager.specParser.getSpecById(task.specId);
  ...
  async buildTransitionContext(specId, fromStatus, toStatus) {
    try {
      ...
      // Get spec progress
      const specProgress = await this.workflowStateManager.getSpecProgress(specId);
      
      // Get project progress for context
      const projectProgress = await this.workflowStateManager.getProjectProgress();
  ...
  async buildCompletionContext(specId, taskId) {
    try {
      ...
      const dependentTasks = allTasks.filter(t => 
        t.depends_on && t.depends_on.includes(taskId)
      );
  ...
  updateMetrics(operation, executionTime, allowed) {
    ...
    this.metrics.averageExecutionTime = 
      (this.metrics.averageExecutionTime * (this.metrics.gateExecutions - 1) + executionTime) / 
      this.metrics.gateExecutions;
...
class AgentCapabilityGateRule extends QualityGateRule {
  ...
  async validateAgentCapabilities(agentType, requirements) {
    ...
    const agentCapabilities = {
      ...
      'backend-specialist': ['api-development', 'database-design', 'integration'],
      'cli-specialist': ['terminal-interfaces', 'user-experience', 'automation']
    };
...
class TaskReadinessGateRule extends QualityGateRule {
  ...
  async validate(operationData) {
    ...
    if (context.currentAssignment && context.currentAssignment.status === 'in_progress') {
      return this.error(
...
class StatusTransitionGateRule extends QualityGateRule {
  ...
  async validate(operationData) {
    ...
    if (toStatus === 'done') {
      const completionCheck = await this.validateCompletionRequirements(context);
  ...
  async validateCompletionRequirements(context) {
    ...
    const incompleteTasks = (spec.tasks || []).filter(task => 
...
commander
  ...
  .action(async () => {
    ...
    console.log(`Block Rate: ${(metrics.blockedOperations / metrics.gateExecutions * 100).toFixed(1)}%`);
    
    if (Object.keys(hookMetrics).length > 0) {
      console.log('\n🪝 Hook Metrics\n');
      for (const [hook, stats] of Object.entries(hookMetrics)) {
        console.log(`${hook}: ${stats.executions} executions, ${stats.averageTime.toFixed(2)}ms avg`);
      }
    }
  });
```
