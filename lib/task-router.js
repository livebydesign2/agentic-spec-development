...
class TaskRecommendationAPI {
  ...
  async initialize() {
    try {
      ...
      console.error(`TaskRecommendationAPI initialization failed: ${error.message}`);
      return false;
    }
  }
  ...
  async getNextTask(options) {
    ...
    try {
      ...
      const recommendation = await this.taskRouter.getNextTask(agentType, enhancedConstraints);
      ...
      if (includeReasoning && result.task) {
        result.reasoning = this.buildDetailedReasoning(result.task, agentType, enhancedConstraints);
      }

      // Add no-task reasoning if no task found
      if (!result.task) {
        result.reasoning = this.buildNoTaskReasoning(recommendation, agentType, enhancedConstraints);
      }
  ...
  async getBatchRecommendations(options) {
    ...
    try {
      ...
      const tasks = await this.taskRouter.getNextTasks(agentType, limit, filters);

      // Apply workload awareness if requested
      let filteredTasks = tasks;
      if (workloadAware && tasks.length > 0) {
        filteredTasks = await this.applyWorkloadAwareness(tasks, agentType, limit);
      }
  ...
  async getTransparentRecommendation(options) {
    ...
    try {
      ...
      const recommendation = await this.taskRouter.getNextTask(agentType, filters);
      ...
      if (includeScoreBreakdown && result.recommendation.task?.scoreBreakdown) {
        result.recommendation.transparency.scoring = result.recommendation.task.scoreBreakdown;
      }

      // Add constraint validation details if requested and available
      if (includeConstraintDetails && result.recommendation.task?.constraintValidation) {
        result.recommendation.transparency.constraints = result.recommendation.task.constraintValidation;
      }
      ...
      result.recommendation.transparency.reasoning = this.buildTransparentReasoning(
        result.recommendation.task,
        agentType,
        filters
      );
  ...
  async getAvailableTasks(options = {}) {
    ...
    try {
      ...
      if (includeBlocked) {
        ...
        tasks = [...tasks, ...blockedTasks.map(task => ({ ...task, blocked: true }))];
      }
  ...
  async validateAssignment(options) {
    ...
    try {
      ...
      if (!validation.details.taskAvailable) {
        ...
        validation.issues.push(`Task ${taskId} is not available (status: ${task.status})`);
      }

      // Check if task is blocked
      if (this.taskRouter.isTaskBlocked(task, allTasks)) {
        validation.valid = false;
        validation.issues.push(`Task ${taskId} is blocked by unmet dependencies`);
      }

      // Check agent capability
      validation.details.agentCapable = this.taskRouter.validateAgentCapability(task, agentType);
      if (!validation.details.agentCapable) {
        validation.valid = false;
        validation.issues.push(`Agent type ${agentType} cannot handle task ${taskId}`);
      }

      // Check constraints using constraint engine
      const constraintValidation = await this.taskRouter.constraintEngine.validateConstraints(
        task, 
        agentType, 
        constraints
      );
  ...
  buildDetailedReasoning(task, agentType, constraints) {
    ...
    if (priority === 'P0') {
      ...
      factors.push({ factor: 'Priority', value: 'P0 (Critical)', impact: 'High' });
    } else if (priority === 'P1') {
      reasons.push('High priority task with significant business impact');
      factors.push({ factor: 'Priority', value: 'P1 (High)', impact: 'Medium' });
    } else {
    ...
    if (task.constraintValidation) {
      ...
      if (validation.isValid) {
        ...
        factors.push({ factor: 'Constraints', value: 'All passed', impact: 'Positive' });
      } else {
        factors.push({ factor: 'Constraints', value: `${validation.violations.length} violations`, impact: 'Negative' });
      }

      if (validation.details.workload && validation.details.workload.workloadStatus) {
        factors.push({ 
          factor: 'Workload', 
          value: validation.details.workload.workloadStatus, 
          impact: validation.details.workload.scoreMultiplier > 1 ? 'Positive' : 'Neutral'
        });
      }
    }
    ...
    if (task.specStatus === 'active') {
      ...
      factors.push({ factor: 'Spec Status', value: 'Active', impact: 'Positive' });
    }
    ...
    if (task.scoreBreakdown) {
      ...
      if (breakdown.contextMatch) {
        factors.push({ 
          ...
          impact: breakdown.contextMatch.multiplier > 1 ? 'Positive' : 'Neutral'
        });
      }
    ...
    return {
      summary: reasons.length > 0 ? reasons.join('; ') : 'Standard task recommendation',
      factors,
      confidence: this.calculateConfidence(factors),
      recommendation: reasons.length >= 3 ? 'Highly Recommended' : 
                     reasons.length >= 2 ? 'Recommended' : 'Available'
    };
  }
  ...
  buildNoTaskReasoning(recommendation, agentType, constraints) {
    ...
    if (constraints.priority) {
      reasons.push(`Filtered by priority: ${Array.isArray(constraints.priority) ? constraints.priority.join(', ') : constraints.priority}`);
    }
    if (constraints.phase) {
      reasons.push(`Filtered by phase: ${Array.isArray(constraints.phase) ? constraints.phase.join(', ') : constraints.phase}`);
    }
    if (constraints.specStatus) {
      reasons.push(`Filtered by spec status: ${Array.isArray(constraints.specStatus) ? constraints.specStatus.join(', ') : constraints.specStatus}`);
    }

    return {
      summary: reasons.join('; '),
      suggestion: 'Try broadening filter criteria or checking if tasks need to be unblocked',
  ...
  buildTransparentReasoning(task, agentType, filters) {
    if (!task) {
      return this.buildNoTaskReasoning({ totalAvailable: 0 }, agentType, filters);
    }
  ...
  analyzeAgentMatch(task, agentType) {
    ...
    if (analysis.requiredContexts.length === 0) {
      ...
      const matchingContexts = analysis.requiredContexts.filter(req =>
        analysis.agentCapabilities.some(cap => 
          cap.includes(req) || req.includes(cap)
        )
      );
      
      const matchRatio = matchingContexts.length / analysis.requiredContexts.length;
      if (matchRatio >= 0.9) {
  ...
  async applyWorkloadAwareness(tasks, agentType, limit) {
    ...
    const currentWorkload = this.taskRouter.constraintEngine.getAgentWorkload(agentType);
    const workloadStats = this.taskRouter.constraintEngine.getWorkloadStats();
    ...
    return tasks.map(task => ({
      ...
      workloadContext: {
        ...
        recommendationReason: currentWorkload < workloadStats.averageWorkload 
