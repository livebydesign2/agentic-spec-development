...
class TaskRouter {
  ...
  extractTaskKeywords(task) {
    ...
    for (const [keyword] of Object.entries(this.taskMatchingKeywords)) {
  ...
  filterByResourceAvailability(tasks, resourceConfig) {
    const { availableResources = {}, allowPartialResources = false } =
      resourceConfig;
   ...
   */
  assessSkillGapSeverity(skill, _task) {
...
class TaskRecommendationAPI {
   ...
   * @private
   */
  buildDetailedReasoning(task, agentType, _constraints) {
    const reasons = [];
    const factors = [];
   ...
   * @private
   */
  async applyWorkloadAwareness(tasks, agentType, _limit) {
