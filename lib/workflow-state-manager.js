...
class WorkflowStateManager {
  ...
  async assignTask(specId, taskId, agentType, options = {}) {
    ...
    try {
      // Validate assignment using quality gates
      const ValidationManager = require('./validation-manager');
      const validationManager = new ValidationManager(this.configManager, this.specParser);
      await validationManager.initialize();

      const qualityGateResult = await validationManager.enforceQualityGate('assignment', {
        specId,
        taskId,
        agentType,
        ...options
      ...
      if (!qualityGateResult.allowed) {
        return {
          success: false,
          error: `Quality gate failed: ${qualityGateResult.reason}`,
          validationErrors: qualityGateResult.errors,
