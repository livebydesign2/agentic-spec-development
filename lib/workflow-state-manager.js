...
class WorkflowStateManager {
  constructor(configManager = null, specParser = null) {
    ...
    this.stateDir = path.join(this.configManager.getProjectRoot(), ".asd", "state");
  ...
  async initialize() {
    ...
    try {
      ...
      throw new Error(`WorkflowStateManager initialization failed: ${error.message}`);
    }
  }
  ...
  async assignTask(specId, taskId, agentType, options = {}) {
    ...
    try {
      ...
      // Update spec frontmatter inline
      await this.updateSpecFrontmatter(specId, {
      ...
      if (totalTime > this.performanceTimeout) {
        console.warn(`Task assignment took ${totalTime}ms, exceeding ${this.performanceTimeout}ms target`);
      }
  ...
  async completeTask(specId, taskId, options = {}) {
    ...
    try {
      ...
      const currentAssignment = assignments.current_assignments[specId]?.[taskId];
      ...
      progress.by_spec[specId].completion_percentage = 
        (progress.by_spec[specId].completed_tasks / progress.by_spec[specId].total_tasks) * 100;
      ...
      const totalTime = Date.now() - startTime;
      if (totalTime > this.performanceTimeout) {
        console.warn(`Task completion took ${totalTime}ms, exceeding ${this.performanceTimeout}ms target`);
      }
  ...
  async completeSubtask(specId, taskId, subtaskId, options = {}) {
    ...
    try {
      ...
      // Update spec frontmatter for subtask
      await this.updateSpecFrontmatter(specId, {
        [`tasks.${taskId}.subtasks.${subtaskId}.status`]: "completed",
        [`tasks.${taskId}.subtasks.${subtaskId}.completed_at`]: new Date().toISOString()
      });
  ...
  async getCurrentAssignments() {
    try {
      ...
      for (const [specId, specAssignments] of Object.entries(assignments.current_assignments)) {
        for (const [taskId, assignment] of Object.entries(specAssignments)) {
          if (assignment.status === "in_progress") {
            ...
            agentWorkloads[agent].total_hours += assignment.estimated_hours || 2;
    ...
    try {
      ...
      for (const phase in phaseProgress) {
        phaseProgress[phase].completion_percentage = 
          (phaseProgress[phase].completed_specs / phaseProgress[phase].specs) * 100;
      }
  ...
  async getProjectProgress() {
    try {
      ...
      for (const specAssignments of Object.values(assignments.current_assignments)) {
  ...
  async getSpecProgress(specId) {
    try {
      ...
      throw new Error(`Failed to get spec progress for ${specId}: ${error.message}`);
    }
  }
  ...
  async detectHandoffs(specId, completedTaskId) {
    try {
      ...
      if (!spec || !spec.tasks) {
        console.warn(`Spec ${specId} or its tasks not found for handoff detection`);
        return null;
      }
      ...
      if (!completedTask) {
        console.warn(`Completed task ${completedTaskId} not found in spec ${specId}`);
        return null;
      }

      // Look for tasks that depend on the completed task
      const dependentTasks = spec.tasks.filter(task => 
        task.depends_on && 
      ...
      if (dependentTasks.length === 0) {
        console.debug(`No dependent tasks found for ${completedTaskId} in ${specId}`);
        return null;
      }
    ...
    try {
      ...
      if (task.depends_on && Array.isArray(task.depends_on) && task.depends_on.length > 0) {
        for (const depId of task.depends_on) {
          const depAssignment = assignments.current_assignments[specId]?.[depId];
   ...
   */
  async updateSpecFrontmatter(specId, updates) {
    try {
      // Find spec file path
      const specs = this.specParser.getSpecs();
      const spec = specs.find(s => s.id === specId);
      ...
      if (!spec || !spec.filePath) {
        console.warn(`Spec file not found for ${specId}, skipping frontmatter update`);
        return false;
      }
      ...
      if (!frontmatterMatch) {
        console.warn(`No frontmatter found in ${spec.filePath}, skipping update`);
        return false;
      }
      ...
      for (const [updatePath, value] of Object.entries(updates)) {
        this.setNestedProperty(frontmatter, updatePath, value);
      }
      ...
      const newFrontmatter = yaml.dump(frontmatter, { indent: 2 });
      ...
      await this.atomicWrite(spec.filePath, newContent);
      
      return true;

    } catch (error) {
      console.error(`Failed to update spec frontmatter for ${specId}: ${error.message}`);
      return false;
    }
  }
  ...
  async syncSpecState(specId) {
    try {
      ...
      const completedTasks = spec.tasks ? 
        spec.tasks.filter(task => task.status === "complete" || task.status === "done").length : 0;
    ...
    try {
      ...
      for (const [specId, specAssignments] of Object.entries(assignments.current_assignments)) {
        for (const [taskId, assignment] of Object.entries(specAssignments)) {
          ...
          if (!assignment.assigned_agent || !assignment.started_at) {
            validation.errors.push(`Invalid assignment structure for ${specId}:${taskId}`);
      ...
      for (const spec of specs) {
        ...
        if (spec.tasks && spec.tasks.length > 0) {
          if (!specProgress) {
            validation.warnings.push(`Missing progress data for spec ${spec.id}`);
      ...
      for (const handoff of handoffs.ready_handoffs) {
        ...
        if (!isValid) {
          validation.warnings.push(`Stale handoff detected: ${handoff.from_task} → ${handoff.to_task}`);
        }
      }
  ...
  async validateHandoff(handoff) {
    try {
      ...
      const assignments = await this.loadState("assignments");
      const currentAssignment = assignments.current_assignments[handoff.spec_id]?.[handoff.to_task];
