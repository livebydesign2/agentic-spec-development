...
class SpecParser {
  ...
  async loadSpecs() {
    ...
    for (const folder of this.statusFolders) {
      ...
      try {
        ...
        const supportedExts = this.adapterFactory.getSupportedFormats()
          .flatMap(format => {
  ...
  getCacheStats() {
    return {
      ...
      cache_hit_rate: this.loadStats.cacheHits + this.loadStats.cacheMisses > 0
        ? Math.round((this.loadStats.cacheHits / (this.loadStats.cacheHits + this.loadStats.cacheMisses)) * 100)
        : 0
    };
  }
