...
class SpecParser {
  ...
  async parseSpecFile(filePath, status) {
    try {
      ...
      // First check YAML frontmatter for ID, then fall back to extraction
      const id = yamlData.id || this.extractId(title, path.basename(filePath));
      ...
      let description =
        yamlData.description || this.extractDescription(contentText);
      const warnings = [];
      ...
      // Parse tasks from both YAML frontmatter and markdown content
      const yamlTasks = yamlData.tasks || [];
      const markdownTasks = this.extractTasks(contentText);
      const tasks = [...yamlTasks, ...markdownTasks];
      ...
      const spec = {
        ...
        filePath, // Add filePath for frontmatter updates
        requiredDocs: yamlData.required_docs || [],
        phase: yamlData.phase,
        created: yamlData.created,
        estimated_hours: yamlData.estimated_hours,
        tags: yamlData.tags || [],
        dependencies: yamlData.dependencies || [],
        acceptance_criteria: yamlData.acceptance_criteria || [],
      };
  ...
  getCacheStats() {
    return {
      ...
      last_load_time: this.lastLoadTime || null,
    };
  }
}
