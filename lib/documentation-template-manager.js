...
class DocumentationTemplateManager {
  constructor(configManager = null, workflowStateManager = null) {
    ...
    this.workflowStateManager = workflowStateManager || new WorkflowStateManager(this.configManager);
  ...
  async initialize() {
    ...
    try {
      ...
      if (initTime > 50) {
        console.warn(`DocumentationTemplateManager initialization took ${initTime}ms`);
      }

      return true;
    } catch (error) {
      throw new Error(`DocumentationTemplateManager initialization failed: ${error.message}`);
    }
  }
  ...
  async createReport(options) {
    ...
    try {
      ...
      const populatedContent = await this.populateTemplate(template, options, reportId);
  ...
  async getSystemStats() {
    ...
    try {
      ...
      for (const report of reports) {
        ...
        // By agent
        stats.reportsByAgent[report.agentType] = (stats.reportsByAgent[report.agentType] || 0) + 1;

        // By spec
        stats.reportsBySpec[report.specId] = (stats.reportsBySpec[report.specId] || 0) + 1;

        // Check if cleanup is upcoming (within 24 hours)
        if (report.retentionHours && report.lifecycle !== 'manual') {
          const cleanupTime = new Date(new Date(report.createdAt).getTime() + (report.retentionHours * 60 * 60 * 1000));
          const hoursUntilCleanup = (cleanupTime - currentTime) / (60 * 60 * 1000);
        ...
        if (!stats.oldestReport || reportTime < new Date(stats.oldestReport.createdAt)) {
          stats.oldestReport = report;
        }
        if (!stats.newestReport || reportTime > new Date(stats.newestReport.createdAt)) {
      ...
      if (reportsWithRetention.length > 0) {
        stats.averageRetentionHours = reportsWithRetention.reduce((sum, r) => sum + r.retentionHours, 0) / reportsWithRetention.length;
      }
...
`
    ...
    return templates[type] || `# ${config.name}\n\nTemplate content for ${type} reports.\n`;
  }
  ...
  async populateTemplate(template, options, reportId) {
    ...
    const replacements = {
      ...
      '{{createdAt}}': new Date().toISOString(),
      '{{reportId}}': reportId,
      '{{context}}': options.context ? JSON.stringify(options.context, null, 2) : ''
    };
  ...
  async setupLifecycleMonitoring() {
    ...
    console.debug('Lifecycle monitoring initialized - use executeCleanup() for cleanup operations');
  }
  ...
  async scheduleCleanup(reportId, metadata) {
    ...
    const cleanupTime = new Date(new Date(metadata.createdAt).getTime() + (metadata.retentionHours * 60 * 60 * 1000));
  ...
  async shouldCleanupReport(report, currentTime, options) {
    ...
    if (report.lifecycle === 'task_completion') {
      if (options.completedTaskIds && options.completedTaskIds.includes(report.taskId)) {
        return true;
      }
    ...
    if (report.lifecycle === 'spec_completion') {
      if (options.completedSpecIds && options.completedSpecIds.includes(report.specId)) {
        return true;
      }
      // Also check retention time
      if (report.retentionHours) {
  ...
  async cleanupSingleReport(reportId) {
    try {
      ...
      const archivePath = path.join(this.reportsDir, 'archived', path.basename(metadata.path));
