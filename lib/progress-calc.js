class ProgressCalculator {
  calculateProgress(feature) {
    ...
    const tasksWithSubtasks = feature.tasks.filter(
      (t) => t && t.subtasks && t.subtasks.length > 0,
    ).length;
    ...
    const completedSubtasks = feature.tasks.reduce(
      (sum, t) =>
        sum + (t && t.subtasks ? t.subtasks.filter((s) => s && s.completed).length : 0),
      0,
    );
  ...
  getInProgressTasks(feature) {
    ...
    return feature.tasks.filter((task) => task && task.status === 'in_progress');
  }
