...
class MarkdownDataAdapter extends BaseDataAdapter {
  ...
  parseContent(content, metadata = {}) {
    ...
    // Extract tasks from both frontmatter and markdown body
    const frontmatterTasks = frontmatter.tasks || [];
    const bodyTasks = this.extractTasks(bodyContent);
    const allTasks = [...frontmatterTasks, ...bodyTasks];
    ...
    const spec = {
      ...
      description: this.extractDescription(bodyContent),
      priority: frontmatter.priority || this.extractPriority(bodyContent),
      tasks: allTasks,
      completedDate: this.extractCompletedDate(bodyContent),
   ...
   * @returns {Object} Object with frontmatter data and bodyContent
   */
  parseFrontmatter(content) {
    ...
    try {
      ...
      console.warn(`Warning: Invalid YAML frontmatter: ${error.message}`);
      return {
        frontmatter: {},
        bodyContent: content,
      };
