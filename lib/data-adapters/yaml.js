...
class YAMLDataAdapter extends BaseDataAdapter {
  ...
  parseContent(content, metadata = {}) {
    try {
      ...
      const spec = {
        ...
        // Completion tracking
        completedDate: this.parseDate(data.completedDate || data.completion_date),
  ...
  parseTasks(tasks) {
    return tasks.map((task, index) => {
      if (typeof task === 'string') {
        ...
        return {
          id: task.match(/TASK-\d+/) ? task.match(/TASK-\d+/)[0] : `TASK-${String(index + 1).padStart(3, '0')}`,
      ...
      return {
        ...
        progress: typeof task.progress === 'number' ? Math.max(0, Math.min(100, task.progress)) : null,
  ...
  addTypeSpecificFields(spec, data) {
    switch (spec.type) {
      case 'BUG':
        ...
        spec.reproductionSteps = this.parseMultilineString(data.reproductionSteps || data.reproduction_steps) || '';
        spec.rootCause = this.parseMultilineString(data.rootCause || data.root_cause) || '';
        spec.proposedSolution = this.parseMultilineString(data.proposedSolution || data.proposed_solution) || '';
        spec.environment = data.environment || '';
        break;

      case 'SPIKE':
        spec.researchType = data.researchType || data.research_type || null;
        spec.researchQuestion = this.parseMultilineString(data.researchQuestion || data.research_question) || '';
        spec.researchFindings = this.parseMultilineString(data.researchFindings || data.research_findings) || '';
        break;
    }
  }
  ...
  serialize(spec) {
    ...
    if (spec.updated) cleanSpec.updated = this.formatDate(spec.updated || new Date().toISOString());
    ...
    if (spec.technical_notes && spec.technical_notes.trim()) {
      cleanSpec.technical_notes = this.formatMultilineString(spec.technical_notes);
    }
  ...
  serializeTasks(tasks) {
    return tasks.map(task => {
      ...
      if (typeof task.progress === 'number') serializedTask.progress = task.progress;
      if (task.started) serializedTask.started = this.formatDate(task.started);
      if (task.completed) serializedTask.completed = this.formatDate(task.completed);
      if (task.estimated_completion) serializedTask.estimated_completion = this.formatDate(task.estimated_completion);
    ...
    switch (spec.type) {
      case 'BUG':
        ...
        if (spec.reproductionSteps && spec.reproductionSteps.trim()) {
          cleanSpec.reproductionSteps = this.formatMultilineString(spec.reproductionSteps);
        }
        ...
        if (spec.proposedSolution && spec.proposedSolution.trim()) {
          cleanSpec.proposedSolution = this.formatMultilineString(spec.proposedSolution);
        }
      ...
      case 'SPIKE':
        ...
        if (spec.researchQuestion && spec.researchQuestion.trim()) {
          cleanSpec.researchQuestion = this.formatMultilineString(spec.researchQuestion);
        }
        if (spec.researchFindings && spec.researchFindings.trim()) {
          cleanSpec.researchFindings = this.formatMultilineString(spec.researchFindings);
        }
        break;
    }
  }
  ...
  validateYAML(content) {
    ...
    if (content.includes('\t')) {
      result.suggestions.push('Consider using spaces instead of tabs for indentation');
    }

    if (!/^---\s*$/m.test(content)) {
      result.suggestions.push('Consider adding document separator (---) at the beginning');
    }
