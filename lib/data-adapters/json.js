...
class JSONDataAdapter extends BaseDataAdapter {
  ...
  addTypeSpecificFields(spec, data) {
    switch (spec.type) {
      case 'BUG':
        ...
        spec.reproductionSteps = data.reproductionSteps || data.reproduction_steps || '';
        spec.rootCause = data.rootCause || data.root_cause || '';
        spec.proposedSolution = data.proposedSolution || data.proposed_solution || '';
        spec.environment = data.environment || '';
        break;

      case 'SPIKE':
        spec.researchType = data.researchType || data.research_type || null;
        spec.researchQuestion = data.researchQuestion || data.research_question || '';
        spec.researchFindings = data.researchFindings || data.research_findings || '';
        break;
    }
  }
  ...
  serialize(spec) {
    ...
    const cleanSpec = {
      ...
      dependencies: spec.dependencies && spec.dependencies.length > 0 ? spec.dependencies : undefined,
      blocking: spec.blocking && spec.blocking.length > 0 ? spec.blocking : undefined,

      // Tasks with full structure
      tasks: spec.tasks && spec.tasks.length > 0 ? this.serializeTasks(spec.tasks) : undefined,

      // Acceptance criteria
      acceptance_criteria: spec.acceptance_criteria && spec.acceptance_criteria.length > 0
        ? spec.acceptance_criteria : undefined,
    ...
    if (this.config.sortKeys) {
      const sortedSpec = {};
      Object.keys(cleanSpec).sort().forEach(key => {
  ...
  addTypeSpecificSerialization(cleanSpec, spec) {
    switch (spec.type) {
      case 'BUG':
        ...
        if (spec.reproductionSteps) cleanSpec.reproductionSteps = spec.reproductionSteps;
        if (spec.rootCause) cleanSpec.rootCause = spec.rootCause;
        if (spec.proposedSolution) cleanSpec.proposedSolution = spec.proposedSolution;
        if (spec.environment) cleanSpec.environment = spec.environment;
        break;

      case 'SPIKE':
        if (spec.researchType) cleanSpec.researchType = spec.researchType;
        if (spec.researchQuestion) cleanSpec.researchQuestion = spec.researchQuestion;
        if (spec.researchFindings) cleanSpec.researchFindings = spec.researchFindings;
        break;
    }
  }
