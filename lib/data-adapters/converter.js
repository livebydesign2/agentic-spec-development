...
class FormatConverter {
  ...
  async convertFile(inputPath, outputPath, options = {}) {
    const {
      sourceFormat = null,
      targetFormat = null,
      _validate = true, // eslint-disable-line no-unused-vars
      _preserveMetadata = true, // eslint-disable-line no-unused-vars
      backup = false,
    } = options;
    ...
    try {
      ...
      const detectedTargetFormat =
        targetFormat || this.getFormatFromExtension(outputPath);
  ...
  async convert(content, sourceFormat, targetFormat, options = {}) {
    ...
    try {
      ...
      const sourceAdapter = new SourceAdapterClass(
      ...
      const targetAdapter = new TargetAdapterClass(
      ...
      return {
        ...
        spec: specToConvert,
        warnings: this.gatherConversionWarnings(
          sourceFormat,
          targetFormat,
          spec
        ),
      ...
      throw new Error(
        `Conversion failed from ${sourceFormat} to ${targetFormat}: ${error.message}`
      );
    }
  }
  ...
  async convertBatch(filePaths, options = {}) {
    ...
    const conversions = filePaths.map((filePath, index) => {
      ...
      if (parallel) {
        return this.convertFile(filePath, outputPath, conversionOptions).then(
          (result) => {
            if (progressCallback) {
  ...
  compareSpecs(spec1, spec2, path = '') {
    ...
    for (const key of keys) {
      ...
      if (!(key in spec1)) {
        differences.push(
          `Missing in original: ${currentPath} = ${JSON.stringify(spec2[key])}`
        );
      } else if (!(key in spec2)) {
        differences.push(
          `Missing in converted: ${currentPath} = ${JSON.stringify(spec1[key])}`
        );
      } else if (typeof spec1[key] !== typeof spec2[key]) {
        differences.push(
          `Type difference at ${currentPath}: ${typeof spec1[
            key
          ]} vs ${typeof spec2[key]}`
        );
        ...
        if (Array.isArray(spec1[key]) && Array.isArray(spec2[key])) {
          if (spec1[key].length !== spec2[key].length) {
            differences.push(
              `Array length difference at ${currentPath}: ${spec1[key].length} vs ${spec2[key].length}`
            );
          } else {
          ...
          differences.push(
            ...this.compareSpecs(spec1[key], spec2[key], currentPath)
          );
        }
      } else if (spec1[key] !== spec2[key]) {
        differences.push(
          `Value difference at ${currentPath}: ${JSON.stringify(
            spec1[key]
          )} vs ${JSON.stringify(spec2[key])}`
        );
      }
    }
  ...
  gatherConversionWarnings(sourceFormat, targetFormat, spec) {
    ...
    if (
      ...
      if (spec.warnings && spec.warnings.length > 0) {
        warnings.push(
          'Some markdown parsing warnings may affect structured format conversion'
        );
      }
    }
    ...
    if (
      ...
      warnings.push(
        'Complex nested structures may be simplified in markdown format'
      );
    }
  ...
  getConversionStats(results) {
    return {
      ...
      withWarnings: results.filter((r) => r.warnings && r.warnings.length > 0)
        .length,
      formats: {
