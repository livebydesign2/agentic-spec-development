...
class SchemaValidator {
  ...
  initializeDefaultSchemas() {
    ...
    this.baseSpecSchema = Joi.object({
      id: Joi.string()
        ...
        .messages({
          'string.pattern.base':
            'ID must follow format: TYPE-000 (e.g., FEAT-001)',
        }),

      type: Joi.string()
        .valid('SPEC', 'FEAT', 'BUG', 'SPIKE', 'MAINT', 'RELEASE')
        .required(),

      status: Joi.string()
        .valid('active', 'backlog', 'done', 'blocked', 'draft')
        .required(),
      ...
      tasks: Joi.array()
        .items(
          Joi.object({
            ...
            status: Joi.string()
              .valid('ready', 'in_progress', 'complete', 'blocked')
              .default('ready'),
  ...
  async validate(spec, options = {}) {
    const {
      schema = 'auto',
      __strict = false, // eslint-disable-line no-unused-vars
      autoFix = false,
      allowUnknown = true,
    } = options;
    ...
    try {
      ...
      const { error, value, warning } = validationSchema.validate(
        spec,
        validationOptions
      );
   ...
   * @private
   */
  formatValidationWarnings(_joiWarning) {
        ...
        case 'string.pattern.base':
          if (error.field === 'id' && error.value) {
            ...
            if (normalized !== error.value) {
              ...
              appliedFixes.push(
                `Normalized ID: ${error.value} → ${normalized}`
              );
            }
          }
          break;
  ...
  async loadCustomSchema(schemaName, schemaPath) {
    try {
      ...
      const joiSchema = Joi.isSchema(schemaData)
        ? schemaData
        : Joi.object(schemaData);

      this.customSchemas.set(schemaName, joiSchema);
      return true;
    } catch (error) {
      throw new Error(
        `Failed to load custom schema ${schemaName}: ${error.message}`
      );
    }
  }
  ...
  generateExample(specType = 'FEAT') {
    const examples = {
      FEAT: {
        ...
        description:
          'This is an example feature specification with all required fields.',
        priority: 'P2',
