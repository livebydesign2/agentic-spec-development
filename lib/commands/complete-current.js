...
class CompleteCurrentCommand {
  ...
  async initialize() {
    ...
    try {
      ...
      this.handoffAutomationEngine = new HandoffAutomationEngine(this.configManager);
      ...
      throw new Error(`CompleteCurrentCommand initialization failed: ${error.message}`);
    }
  }
  ...
  async execute(options) {
    ...
    try {
      ...
      console.log(chalk.white(`Task: ${chalk.cyan(specId)}:${chalk.yellow(taskId)}`));
      ...
      if (!options.skipLint) {
        ...
        if (!lintResult.success) {
          return this.createErrorResult(`Linting failed: ${lintResult.error}`, startTime);
        }
      }
      ...
      if (!options.skipTests) {
        ...
        if (!testResult.success) {
          return this.createErrorResult(`Tests failed: ${testResult.error}`, startTime);
        }
      }
      ...
      if (!options.skipCommit) {
        ...
        if (!gitResult.success) {
          console.warn(chalk.yellow(`⚠️  Git workflow failed: ${gitResult.error}`));
      ...
      this.displaySuccessMessage(specId, taskId, completionResult, handoffResult, gitResult);
  ...
  async determineTaskToComplete(options) {
    try {
      ...
      if (options.task) {
        ...
        const assignments = await this.workflowStateManager.getCurrentAssignments();
        const targetAssignment = assignments.current_assignments.find(a =>
      ...
      const assignments = await this.workflowStateManager.getCurrentAssignments();
      const activeAssignments = assignments.current_assignments.filter(a => a.status === 'in_progress');
      ...
      if (activeAssignments.length > 1) {
        ...
        activeAssignments.forEach((assignment, index) => {
          console.log(chalk.white(`  ${index + 1}. ${assignment.spec_id}:${assignment.task_id} (${assignment.assigned_agent})`));
        });
        return this.createErrorResult(
      ...
      return this.createErrorResult(`Failed to determine task to complete: ${error.message}`);
    }
  }
  ...
  async executeLinting() {
    ...
    try {
      ...
      console.log(chalk.yellow('⚠️  Linting failed, attempting automatic fixes...'));
  ...
  async completeTask(specId, taskId, options) {
    ...
    try {
      const completionResult = await this.workflowStateManager.completeTask(
        ...
        taskId,
        {
          notes: options.notes || 'Task completed via complete-current automation',
      ...
      if (!completionResult.success) {
        ...
        return this.createErrorResult(`Task completion failed: ${completionResult.error}`);
      }
      ...
      return this.createErrorResult(`Failed to complete task: ${error.message}`);
    }
  }
  ...
  async executeGitWorkflow(specId, taskId, options) {
    ...
    try {
      ...
      const commitMessage = options.commitMessage || this.generateCommitMessage(specId, taskId);
  ...
  async createCommitWithRetry(commitMessage) {
    ...
    while (attempt <= maxRetries) {
      try {
        console.log(chalk.blue(`🔄 Commit attempt ${attempt}/${maxRetries}...`));
        ...
        if (commitResult.success) {
          ...
          return {
            success: true,
            commitHash: hashResult.success ? hashResult.output.trim() : 'unknown',
            attempt
          };
        }
        ...
        if (commitResult.output && commitResult.output.includes('pre-commit')) {
          console.log(chalk.yellow('⚠️  Pre-commit hook modified files, re-staging...'));
        ...
        console.log(chalk.yellow(`⚠️  Commit attempt ${attempt} failed, retrying...`));
        attempt++;
      }
    }
  ...
  async triggerHandoffAutomation(specId, taskId) {
    ...
    try {
      ...
      const handoffResult = await this.handoffAutomationEngine.executeHandoff(handoffTrigger);

      if (handoffResult.success && handoffResult.handoffNeeded) {
        console.log(chalk.green('✅ Handoff executed successfully'));
        console.log(chalk.white(`Next task: ${chalk.cyan(handoffResult.nextTask)} → ${chalk.yellow(handoffResult.nextAgent)}`));
        ...
        console.warn(chalk.yellow(`⚠️  Handoff failed: ${handoffResult.error}`));
      }

      return handoffResult;

    } catch (error) {
      console.warn(chalk.yellow(`⚠️  Handoff automation failed: ${error.message}`));
      return {
        success: false,
        error: error.message
      };
...
Co-Authored-By: Claude <noreply@anthropic.com>`;
  ...
  async getGitStatus() {
    try {
      ...
      console.warn(chalk.yellow(`⚠️  Failed to get git status: ${error.message}`));
      return [];
    }
  }
   ...
   */
  displaySuccessMessage(specId, taskId, completionResult, handoffResult, gitResult) {
    console.log(chalk.green('\n🎉 Task Successfully Completed!'));
    console.log(chalk.gray('='.repeat(50)));
    console.log(chalk.white(`Task: ${chalk.cyan(specId)}:${chalk.yellow(taskId)}`));
    console.log(chalk.white(`Status: ${chalk.green('complete')}`));
    console.log(chalk.white(`Completed: ${completionResult.completion.completed_at}`));

    if (completionResult.completion.duration_hours) {
      console.log(chalk.white(`Duration: ${completionResult.completion.duration_hours} hours`));
    }

    if (gitResult && gitResult.success) {
      console.log(chalk.white(`Commit: ${chalk.cyan(gitResult.commitHash || 'created')}`));
    }

    console.log(chalk.blue('\n📝 Next Steps:'));

    if (handoffResult && handoffResult.success && handoffResult.handoffNeeded) {
      console.log(chalk.white(`  1. Next task ready: ${chalk.cyan(handoffResult.nextTask)} → ${chalk.yellow(handoffResult.nextAgent)}`));
    ...
    console.log(chalk.white('  2. Check project progress with workflow status'));
