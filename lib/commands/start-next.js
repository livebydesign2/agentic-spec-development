...
class StartNextCommand {
  ...
  async initialize() {
    ...
    try {
      ...
      throw new Error(
        `StartNextCommand initialization failed: ${error.message}`
      );
    }
  }
    ...
    try {
      ...
      if (!options.agent) {
        return this.createErrorResult(
          'Agent type is required (use --agent <agent-type>)',
          startTime
        );
      }
      ...
      if (!task) {
        return this.createResult({
          ...
          suggestions: this.generateNoTaskSuggestions(
            options.agent,
      ...
      console.log(
        chalk.green(
          `\n✅ Recommended Task: ${chalk.cyan(task.specId)}:${chalk.yellow(
            task.id
          )}`
        )
      );
      ...
      if (task.specPriority || task.priority) {
        console.log(
          chalk.white(
            `Priority: ${chalk.red(task.specPriority || task.priority)}`
          )
        );
      }
  ...
  async getTaskRecommendation(options) {
    try {
      ...
      if (!result.success) {
        ...
        return this.createErrorResult(
          `Task recommendation failed: ${result.error}`
        );
      }
      ...
      return this.createErrorResult(
        `Failed to get task recommendation: ${error.message}`
      );
    }
  }
    ...
    try {
      ...
      if (!validation.isValid && !validation.canProceed) {
        const actionableErrors =
          this.assignmentValidator.getActionableErrors(validation);
  ...
  async executeAssignment(task, options) {
    try {
      ...
      if (!result.success) {
        return this.createErrorResult(
          `Task assignment failed: ${result.error}`
        );
      }
      ...
      return this.createErrorResult(
        `Failed to execute assignment: ${error.message}`
      );
    }
  }
  ...
  displaySuccessMessage(task, assignmentResult) {
    ...
    console.log(
      chalk.white(`Task: ${chalk.cyan(task.specId)}:${chalk.yellow(task.id)}`)
    );
    console.log(chalk.white(`Status: ${chalk.green('in_progress')}`));
    console.log(
      chalk.white(`Assigned: ${assignmentResult.assignment.started_at}`)
    );
  ...
  generateNoTaskSuggestions(agentType, recommendationResult) {
    ...
    if (metadata && metadata.totalAvailable === 0) {
      ...
      suggestions.push(
        'Try broadening agent specializations or check other agent types'
      );
      suggestions.push(
      ...
      suggestions.push(
        '  • Include backlog specs (--spec-status active,ready,backlog)'
      );
    }
  ...
  createDryRunResult(task, validation, startTime) {
    ...
    console.log(
      chalk.white(
        `Would assign: ${chalk.cyan(task.specId)}:${chalk.yellow(task.id)}`
      )
    );
    console.log(
      chalk.white(`To agent: ${chalk.yellow(task.agent || 'unspecified')}`)
    );
    console.log(
      chalk.white(
        `Validation: ${
          validation.isValid ? chalk.green('PASS') : chalk.red('FAIL')
        }`
      )
    );
    ...
    if (validation.confidence < 1.0) {
      console.log(
        chalk.yellow(`Confidence: ${Math.round(validation.confidence * 100)}%`)
      );
    }
