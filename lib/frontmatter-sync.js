...
class FrontmatterSync {
  ...
  async updateSpecFrontmatter(specFilePath, updates, options = {}) {
    ...
    try {
      ...
      const { frontmatter, content, originalContent } =
        await this.parseSpecFile(specFilePath);
      ...
      const updatedFrontmatter = await this.applyUpdates(
        frontmatter,
        updates,
        options
      );
      ...
      const validation = await this.validateFrontmatter(
        updatedFrontmatter,
        specFilePath
      );
      ...
      const newContent = await this.reconstructFileContent(
        content,
        updatedFrontmatter
      );
      ...
      if (totalTime > this.performanceTarget) {
        console.warn(
          `Frontmatter update took ${totalTime}ms, exceeding ${this.performanceTarget}ms target`
        );
      }
    ...
    try {
      ...
      if (!allSuccessful && options.rollbackOnError) {
        for (const { filePath, backupPath } of backups) {
          try {
            ...
            console.error(
              `Failed to restore ${filePath} from backup: ${error.message}`
            );
          }
        }
      }
      ...
      for (const { filePath, backupPath } of backups) {
        try {
          ...
          console.error(
            `Failed to restore ${filePath}: ${recoveryError.message}`
          );
        }
      }
   ...
   */
  async validateFrontmatter(frontmatter, _filePath) {
    const validation = {
      isValid: true,
      errors: [],
      warnings: [],
    ...
    if (
      frontmatter.type &&
      !['FEAT', 'BUG', 'SPIKE', 'MAINT', 'RELEASE'].includes(frontmatter.type)
    ) {
    ...
    if (
      frontmatter.status &&
      !['active', 'backlog', 'done', 'blocked'].includes(frontmatter.status)
    ) {
    ...
    if (
      ...
      !['P0', 'P1', 'P2', 'P3'].includes(frontmatter.priority)
    ) {
      ...
      for (let i = 0; i < frontmatter.tasks.length; i++) {
        ...
        if (!task.id || !task.title) {
          validation.errors.push(
            `Task ${i} missing required fields (id, title)`
          );
          validation.isValid = false;
        }
      }
