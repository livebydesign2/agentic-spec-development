...
class ConfigManager {
  ...
  loadConfig() {
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        console.warn(
          chalk.yellow('⚠ Configuration file not found, using defaults')
        );
        ...
        console.error(
          chalk.yellow('   • Check asd.config.js for syntax errors')
        );
        console.error(chalk.yellow('   • Validate JavaScript syntax'));
        console.error(chalk.yellow('   • Remove config file to use defaults'));
        console.error(
          chalk.yellow('   • Using fallback configuration for now')
        );
      } else if (error.code === 'EACCES' || error.code === 'EPERM') {
        console.error(
          chalk.red('❌ Permission denied reading configuration file')
        );
        console.error(
          chalk.yellow('   Check file permissions on asd.config.js')
        );
        console.error(chalk.yellow('   Using default configuration'));
      } else {
        console.warn(
          chalk.yellow('⚠ Failed to load configuration:'),
          error.message
        );
  ...
  getDefaultConfig() {
    try {
      ...
      const chalk = require('chalk');
      console.warn(
        chalk.yellow('⚠ Could not load default config file:'),
        error.message
      );
      ...
      if (error.code === 'ENOENT') {
        console.warn(
          chalk.yellow('   Default config file is missing from installation')
        );
      } else if (error instanceof SyntaxError) {
        console.warn(
          chalk.yellow('   Default config file has invalid JSON syntax')
        );
      }
    }
