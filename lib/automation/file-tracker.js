...
class FileTracker extends EventEmitter {
  ...
  async startTracking(options = {}) {
    try {
      ...
      console.log(
        `📁 File tracking started - monitoring ${patterns.length} patterns`
      );
  ...
  async performInitialScan() {
    try {
      ...
      if (!gitStatus.success) {
        console.warn(
          `⚠️  Could not get git status for initial scan: ${gitStatus.error}`
        );
        return;
      }
      ...
      console.log(
        `📁 Initial scan complete - found ${
          gitStatus.detailedFiles?.length || 0
        } existing changes`
      );
    } catch (error) {
  ...
  handleFileChange(filePath, eventType) {
    try {
      ...
      if (this.trackedChanges.size >= this.options.maxFiles) {
        console.warn(
          `⚠️  Maximum tracked files limit (${this.options.maxFiles}) reached`
        );
        return;
      }
  ...
  async processBatchChanges() {
    ...
    try {
      ...
      if (!gitStatus.success) {
        console.warn(
          `⚠️  Could not update git status for batch changes: ${gitStatus.error}`
        );
        return;
      }
      ...
      this.emit('batch_processed', {
        ...
        gitTrackedFiles: filesToProcess.filter(
          (f) => this.trackedChanges.get(f)?.isGitTracked
        ).length,
      });
    } catch (error) {
  ...
  async stopTracking() {
    try {
      ...
      this.statistics.averageSessionDuration =
        (this.statistics.averageSessionDuration *
          (this.statistics.sessionsStarted - 1) +
          sessionDuration) /
        this.statistics.sessionsStarted;
      ...
      console.log(
        `📁 File tracking stopped - ${
          result.fileCount
        } files tracked over ${Math.round(sessionDuration / 1000)}s`
      );
