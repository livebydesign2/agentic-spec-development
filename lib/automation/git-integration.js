...
class GitIntegration extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      console.error(`❌ GitIntegration initialization failed: ${error.message}`);
      return false;
    }
  }
  ...
  async validateRepository() {
    try {
      ...
      const repositoryInfo = {
        ...
        remotes: this.parseRemotes(remoteResult.success ? remoteResult.output : ''),
  ...
  async startFileTracking(options = {}) {
    try {
      ...
      console.log(`📁 File tracking started - monitoring ${statusResult.files.length} existing files`);
  ...
  async stopFileTracking() {
    try {
      ...
      // Get final status
      const statusResult = await this.getGitStatus();
      const trackedFiles = statusResult.success ? statusResult.files : Array.from(this.workspaceState.trackedFiles);
      ...
      console.log(`📁 File tracking stopped - ${trackedFiles.length} files tracked`);
  ...
  async stageFiles(files = '.', options = {}) {
    try {
      ...
      if (files !== '.' && !skipValidation) {
        const validation = await this.validateFilesExist(Array.isArray(files) ? files : [files]);
      ...
      const addResult = await this.runGitCommand(gitArgs, {
        description: `Stage files: ${Array.isArray(files) ? files.join(', ') : files}`
      });
  ...
  async createCommit(message, options = {}) {
    try {
      ...
      while (attempt <= maxRetries) {
        try {
          console.log(`🔄 Creating commit (attempt ${attempt}/${maxRetries})...`);
          ...
          if (retryOnHookFailure && this.isPreCommitHookFailure(commitResult.output)) {
            console.log('⚠️  Pre-commit hook modified files, re-staging and retrying...');

            // Re-stage files that may have been modified by hooks
            const restageResult = await this.stageFiles('.', { allowEmpty: true });
            if (!restageResult.success) {
              throw new Error(`Failed to re-stage files after hook: ${restageResult.error}`);
            }

            attempt++;
            lastError = `Pre-commit hook failure (attempt ${attempt - 1}): ${commitResult.output}`;
            continue;
          }
  ...
  async runGitCommand(args, options = {}) {
    ...
    try {
      ...
      const operationId = `git_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  executeCommand(command, args, options = {}) {
    return new Promise((resolve) => {
      const {
        description: __description = 'Command execution',
        timeout = this.options.commandTimeout,
        cwd = process.cwd()
      } = options;
