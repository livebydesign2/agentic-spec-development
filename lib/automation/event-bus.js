...
class EventBus extends EventEmitter {
  ...
  registerHandler(eventType, handler, options = {}) {
    ...
    console.log(
      `📝 Registered handler ${handlerId} for event type: ${eventType}`
    );
    return handlerId;
  }
  ...
  unregisterHandler(handlerId) {
    ...
    this.removeListener(
      handlerMetadata.eventType,
      handlerMetadata.wrappedHandler
    );
  ...
  async emitEnhanced(eventType, eventData = {}, options = {}) {
    const __startTime = Date.now(); // eslint-disable-line no-unused-vars
    ...
    try {
      ...
      console.log(
        `📤 Queued event: ${eventType} (priority: ${enhancedEvent.metadata.priority})`
      );
      return true;
    } catch (error) {
  ...
  async processEvent(event) {
    ...
    try {
      // Check timeout
      const eventAge =
        Date.now() - new Date(event.metadata.timestamp).getTime();
      ...
      if (processingTime > this.options.performanceTarget) {
        console.warn(
          `⚠️ Event processing took ${processingTime}ms, exceeding ${this.options.performanceTarget}ms target`
        );
      }
    ...
    const wrappedHandler = async (eventData, eventMetadata) => {
      ...
      if (circuitBreaker.state === 'open') {
        ...
        if (now < circuitBreaker.nextAttemptTime) {
          console.warn(
            `🔴 Handler ${handlerMetadata.id} circuit breaker is OPEN, skipping execution`
          );
          return;
        } else {
          ...
          console.log(
            `🟡 Handler ${handlerMetadata.id} circuit breaker is HALF-OPEN, attempting execution`
          );
        }
      }
      ...
      try {
        ...
        const handlerPromise = Promise.resolve(
          handlerMetadata.handler(eventData, eventMetadata)
        );
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(
            () => reject(new Error('Handler timeout')),
            handlerMetadata.timeout
          );
        });
        ...
        handlerMetadata.averageExecutionTime =
          (handlerMetadata.averageExecutionTime *
            (handlerMetadata.totalExecutions - 1) +
            executionTime) /
          handlerMetadata.totalExecutions;
        ...
        if (circuitBreaker.state === 'half-open') {
          ...
          console.log(
            `🟢 Handler ${handlerMetadata.id} circuit breaker is CLOSED`
          );
        }
        ...
        const handlerMetrics = this.metrics.handlerPerformance.get(
          handlerMetadata.id
        );
        handlerMetrics.totalExecutions++;
        handlerMetrics.totalTime += executionTime;
        handlerMetrics.averageTime =
          handlerMetrics.totalTime / handlerMetrics.totalExecutions;
      } catch (error) {
        console.error(
          `❌ Handler ${handlerMetadata.id} failed: ${error.message}`
        );
        ...
        if (circuitBreaker.failures >= this.options.circuitBreakerThreshold) {
          ...
          circuitBreaker.nextAttemptTime =
            Date.now() + this.options.circuitBreakerResetTime;
          console.warn(
            `🔴 Handler ${handlerMetadata.id} circuit breaker is OPEN due to ${circuitBreaker.failures} failures`
          );
        }
  ...
  handleEventProcessingError(event, error) {
    ...
    if (event.metadata.attempt < event.metadata.maxAttempts) {
      ...
      console.log(
        `🔄 Retrying event ${event.type} (attempt ${event.metadata.attempt}/${event.metadata.maxAttempts})`
      );
  ...
  moveToDeadLetterQueue(event, reason) {
    ...
    console.warn(
      `💀 Moved event to dead letter queue: ${event.type} (reason: ${reason})`
    );
    ...
    return {
      ...
      handlers: {
        ...
        performance: Object.fromEntries(
          this.metrics.handlerPerformance.entries()
        ),
  ...
  getEventHistory(filters = {}) {
    ...
    if (filters.since) {
      ...
      history = history.filter(
        (event) => new Date(event.metadata.timestamp) >= sinceTime
      );
    }
  ...
  async replayDeadLetterEvents(eventIds = null) {
    ...
    for (const dlqEvent of eventsToReplay) {
      ...
      const dlqIndex = this.deadLetterQueue.findIndex(
        (e) => e.metadata.id === dlqEvent.metadata.id
      );
      if (dlqIndex !== -1) {
  ...
  async shutdown() {
    ...
    if (this.eventQueue.length > 0) {
      console.log(
        `📤 Processing ${this.eventQueue.length} remaining events...`
      );
