...
class ErrorResolver extends EventEmitter {
  ...
  initializeResolutionPatterns() {
    return {
      linting: {
        ...
        'Unused variable': {
          pattern:
            /(unused|defined but never used)\s+(variable|var|let|const)/i,
          resolution: 'manual',
      ...
      for (const [patternName, patternInfo] of Object.entries(allPatterns)) {
        if (patternInfo.pattern.test(error)) {
          ...
          if (
            ...
            match.confidence >= this.options.confidenceThreshold
          ) {
    ...
    try {
      ...
      for (const resolution of analysis.resolutions) {
        ...
        if (
          ...
          resolution.confidence >= this.options.confidenceThreshold
        ) {
  ...
  getFormattedSuggestions(analysis) {
      ...
      formatted.push({
        ...
        tools: analysis.tools.filter((tool) =>
          analysis.resolutions.some(
            (r) => r.resolution === 'automatic' && r.tools.includes(tool)
          )
        ),
      });
    }
    ...
    if (analysis.manualInterventionRequired) {
      const manualSuggestions = analysis.suggestions.filter((s) =>
        analysis.resolutions.some(
          (r) => r.resolution === 'manual' && r.suggestions.includes(s)
        )
      );
  ...
  getStatistics() {
    return {
      ...
      patternCount: Object.keys(this.resolutionPatterns).reduce(
        (total, category) =>
          total + Object.keys(this.resolutionPatterns[category]).length,
        0
      ),
      successRate:
        this.statistics.totalResolutionAttempts > 0
