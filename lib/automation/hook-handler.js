...
class HookHandler extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      const _hooksDetected = await this.detectGitHooks();
      ...
      console.log(`✅ HookHandler initialized - detected ${this.detectedHooks.size} hooks`);
  ...
  async detectPackageJsonHooks() {
    ...
    try {
      ...
      if (packageJson.husky && packageJson.husky.hooks) {
        for (const [hookName, command] of Object.entries(packageJson.husky.hooks)) {
    ...
    for (const [hookName, hookInfo] of this.detectedHooks) {
      try {
        ...
        if (analysis.complexity === 'high') {
          analysis.recommendations.push('Consider splitting into multiple hooks');
        }
        if (analysis.tools.includes('npm') && analysis.tools.includes('yarn')) {
          analysis.recommendations.push('Standardize on single package manager');
        }
        ...
        console.warn(`⚠️  Hook analysis failed for ${hookName}: ${error.message}`);
      }
    }
  }
    ...
    try {
      ...
      while (attempt <= maxRetries) {
        ...
        if (failureAnalysis.retryable && attempt < maxRetries) {
          console.log(`🔄 Hook failure is retryable: ${failureAnalysis.reason}`);

          // Apply recovery actions if available
          if (failureAnalysis.recovery) {
            const recoveryResult = await this.applyRecoveryAction(failureAnalysis.recovery, executionResult);
            if (recoveryResult.success) {
              console.log(`🔧 Recovery action applied: ${recoveryResult.action}`);
            }
          }
      ...
      this.updateMetrics(hookName, finalResult.success, executionTime, finalResult);
  ...
  analyzeHookFailure(executionResult) {
    ...
    switch (analysis.category) {
      case 'formatting':
        analysis.suggestions.push('Files were reformatted - re-staging required');
        break;
      case 'linting':
   ...
   */
  async applyRecoveryAction(recoveryAction, executionResult) {
    try {
  ...
  updateMetrics(hookName, success, executionTime, result) {
    ...
    this.metrics.averageExecutionTime =
      (this.metrics.averageExecutionTime * (this.metrics.totalHookExecutions - 1) + executionTime) /
      this.metrics.totalHookExecutions;
  ...
  getStatistics() {
    return {
      metrics: {
        ...this.metrics,
        hookPerformance: Object.fromEntries(this.metrics.hookPerformance.entries())
      },
      hooks: {
        detected: this.detectedHooks.size,
        byType: this.getHooksByType(),
        configurations: Array.from(this.detectedHooks.entries()).map(([name, info]) => ({
          name,
          type: info.type,
