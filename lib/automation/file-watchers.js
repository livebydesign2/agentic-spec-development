...
class FileWatchers extends EventEmitter {
  ...
  async initialize() {
    try {
      const projectRoot = this.configManager
  ...
  async processFileChange(
    ...
    try {
      ...
      if (detectionDelay > this.performanceTarget) {
        console.warn(
          `⚠️ File change detection took ${detectionDelay}ms, exceeding ${this.performanceTarget}ms target`
        );
      }
      ...
      this.watcherState[`${watcherType}Watcher`].lastEvent =
        changePayload.timestamp;
      ...
      console.log(
        `📝 ${watcherType.toUpperCase()} ${changeType}: ${path.basename(
          filePath
        )} (${detectionDelay}ms)`
      );
    } catch (error) {
  ...
  async createChangePayload(
    ...
    const payload = {
      ...
      relativePath: this.configManager
        ? path.relative(this.configManager.getProjectRoot(), filePath)
        : filePath,

      // Event tracking
      eventId: `${watcherType}_${changeType}_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`,
    };
  ...
  performHealthCheck() {
    const healthStatus = {
      ...
      errorRate:
        this.eventStats.totalEvents > 0
          ? (this.eventStats.errorEvents / this.eventStats.totalEvents) * 100
          : 0,
    ...
    if (
      !this.watcherState.yamlWatcher.active &&
      !this.watcherState.yamlWatcher.error
    ) {
    ...
    if (
      !this.watcherState.jsonWatcher.active &&
      !this.watcherState.jsonWatcher.error
    ) {
  ...
  async restartWatcher(watcherType) {
    try {
      ...
      console.log(
        `✅ ${watcherType.toUpperCase()} watcher restarted successfully`
      );
      ...
      console.error(
        `❌ Failed to restart ${watcherType} watcher: ${error.message}`
      );
  ...
  getStatus() {
    return {
      ...
      performance: {
        ...
        averageDetectionTime: this.eventStats.averageDetectionTime,
        withinTarget:
          this.eventStats.averageDetectionTime <= this.performanceTarget,
      },
      statistics: {
        ...this.eventStats,
