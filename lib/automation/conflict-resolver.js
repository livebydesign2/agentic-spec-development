...
class ConflictResolver extends EventEmitter {
    ...
    this.resolutionStrategies = {
      timestamp_based: {
        ...
        applicable_conflicts: [
          ...
          'assignment_race_condition',
        ],
      },
      priority_based: {
        description:
          'Resolve based on source priority (YAML > JSON for specs, JSON > YAML for operational data)',
      ...
      business_logic: {
        ...
        applicable_conflicts: [
          ...
          'assignment_validation_conflict',
        ],
      },
      manual_intervention: {
    ...
    this.confidenceFactors = {
      ...
      source_authority: {
        ...
        calculate: (conflictType, sourceType) => {
          if (
            (conflictType.includes('spec') || conflictType.includes('task')) &&
            sourceType === 'yaml'
          ) {
            return 1.0;
          }
          if (
            (conflictType.includes('assignment') ||
              conflictType.includes('progress')) &&
            sourceType === 'json'
          ) {
            return 1.0;
          }
  ...
  async initialize() {
    try {
      ...
      console.error(
        `❌ ConflictResolver initialization failed: ${error.message}`
      );
  ...
  registerEventHandlers() {
    ...
    this.eventBus.registerHandler(
      'validation_complete',
      async (eventData, eventMetadata) => {
        if (eventData.conflicts && eventData.conflicts.length > 0) {
          await this.handleValidationConflicts(
            eventData.conflicts,
            eventData,
            eventMetadata
          );
  ...
  async resolveConflict(conflict, options = {}) {
    ...
    try {
      ...
      const selectedStrategy = this.selectResolutionStrategy(
        strategyScores,
        conflict
      );
      ...
      console.log(
        ...
        } (confidence: ${selectedStrategy.confidence.toFixed(2)})`
      );

      // Check if we can auto-resolve with sufficient confidence
      if (
        selectedStrategy.strategy.auto_resolve &&
        selectedStrategy.confidence >=
          selectedStrategy.strategy.confidence_threshold
      ) {
        ...
        if (resolutionResult.success) {
          ...
          resolutionRecord.duration =
            resolutionRecord.endTime - resolutionRecord.startTime;
          ...
          console.log(
            `✅ Conflict resolved automatically: ${conflictId} (${resolutionRecord.duration}ms)`
          );
          ...
          throw new Error(
            `Automatic resolution failed: ${resolutionResult.error}`
          );
        }
      } else {
        ...
        console.log(
          `👤 Conflict queued for manual intervention: ${conflictId}`
        );
      ...
      console.error(
        `❌ Conflict resolution failed: ${conflictId}: ${error.message}`
      );
  ...
  async calculateStrategyConfidence(conflict) {
    ...
    for (const [strategyName, strategy] of Object.entries(
      ...
      if (
        !strategy.applicable_conflicts.some(
          (pattern) => conflict.type.includes(pattern) || pattern === 'all'
        )
      ) {
      ...
      for (const [factorName, factor] of Object.entries(
        this.confidenceFactors
      )) {
        let factorScore = 0;
   ...
   */
  selectResolutionStrategy(strategyScores, _conflict) {
   ...
   * @returns {Promise<Object>} Resolution result
   */
  async executeAutomaticResolution(
    conflict,
    selectedStrategy,
    resolutionRecord
  ) {
    try {
      ...
      switch (selectedStrategy.name) {
        case 'timestamp_based':
          resolutionResult = await this.resolveByTimestamp(
            conflict,
            resolutionRecord
          );
          break;
        ...
        case 'priority_based':
          resolutionResult = await this.resolveBySourcePriority(
            conflict,
            resolutionRecord
          );
          break;
        ...
        case 'business_logic':
          resolutionResult = await this.resolveByBusinessLogic(
            conflict,
            resolutionRecord
          );
          break;
        ...
        default:
          throw new Error(
            `Unknown automatic resolution strategy: ${selectedStrategy.name}`
          );
      }
  ...
  async resolveByTimestamp(conflict, resolutionRecord) {
    try {
      if (!conflict.timestamps || !conflict.yamlData || !conflict.jsonData) {
        throw new Error(
          'Insufficient timestamp data for timestamp-based resolution'
        );
      }
      ...
      const useYamlData =
        ...
        new Date(conflict.timestamps.yaml || 0).getTime();
      const sourceData = useYamlData ? conflict.yamlData : conflict.jsonData;
      const targetType = useYamlData ? 'json' : 'yaml';

      console.log(
        `🕒 Resolving by timestamp: using ${
          useYamlData ? 'YAML' : 'JSON'
        } data (newer by ${Math.round(
          (newerTimestamp - olderTimestamp) / 1000
        )}s)`
      );
  ...
  async resolveBySourcePriority(conflict, resolutionRecord) {
    try {
      ...
      const isSpecStructureConflict =
        conflict.type.includes('spec') ||
      ...
      console.log(
        `📋 Resolving by source priority: using ${
          useYamlData ? 'YAML' : 'JSON'
        } as authority for ${conflict.type}`
      );
  ...
  async resolveByBusinessLogic(conflict, resolutionRecord) {
    try {
      ...
      if (
        conflict.type.includes('assignment') &&
        conflict.type.includes('validation')
      ) {
        // For assignment validation conflicts, check if assignment is still valid
        resolutionApproach = await this.resolveAssignmentValidationConflict(
          conflict
        );
        ...
        throw new Error(
          `No business logic rule defined for conflict type: ${conflict.type}`
        );
      }
      ...
      if (resolutionApproach.success) {
        ...
        throw new Error(
          `Business logic resolution failed: ${resolutionApproach.error}`
        );
      }
    } catch (error) {
      return { success: false, error: error.message };
      ...
      if (targetType === 'json') {
        ...
        if (this.frontmatterSync && this.workflowStateManager) {
          ...
          if (sourceData.completion_notes) {
            ...
            changes.push({
              type: 'notes_sync',
              value: sourceData.completion_notes,
            });
          }

          if (Object.keys(yamlUpdates).length > 0) {
            const updateResult =
              await this.workflowStateManager.updateSpecFrontmatter(
                specId,
                yamlUpdates,
                { automated: true, conflictResolution: resolutionRecord.id }
              );
   ...
   */
  async queueForManualIntervention(
    conflict,
    recommendedStrategy,
    resolutionRecord
  ) {
  ...
  async processManualResolution(manualRequestId, resolution) {
    ...
    try {
      ...
      const requestIndex = this.manualInterventionQueue.findIndex(
        (req) => req.id === manualRequestId
      );
      if (requestIndex === -1) {
      ...
      console.log(
        `👤 Processing manual resolution for conflict: ${manualRequest.conflictId}`
      );

      // Apply the manual resolution
      const applicationResult = await this.applyManualResolution(
        manualRequest,
        resolution
      );
      ...
      if (applicationResult.success) {
        ...
        console.log(
          `✅ Manual resolution applied: ${manualRequestId} (${duration}ms)`
        );
        ...
        throw new Error(
          `Manual resolution application failed: ${applicationResult.error}`
        );
      }
    } catch (error) {
      ...
      performance: {
        ...
        automaticResolutionRate:
          this.metrics.totalConflicts > 0
            ? this.metrics.automaticResolutions / this.metrics.totalConflicts
            : 0,
        highConfidenceRate:
          this.metrics.totalConflicts > 0
  ...
  generateConflictId(conflict) {
    return `conflict_${conflict.type}_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  }
  ...
  async createConflictBackup(resolutionRecord) {
    console.log(
      `💾 Creating backup for conflict resolution: ${resolutionRecord.id}`
    );
  ...
  updateMetrics(type, success, duration, confidence) {
    ...
    this.metrics.averageResolutionTime =
      (this.metrics.averageResolutionTime * (this.metrics.totalConflicts - 1) +
        duration) /
      this.metrics.totalConflicts;
  ...
  async handleValidationConflicts(conflicts, validationData, eventMetadata) {
    for (const conflict of conflicts) {
      try {
        ...
        console.error(
          `❌ Failed to handle validation conflict: ${error.message}`
        );
      }
    }
  }
  ...
  async shutdown() {
    ...
    if (this.activeConflicts.size > 0) {
      console.log(
        `⏳ ${this.activeConflicts.size} conflicts still active during shutdown`
      );
    }

    if (this.manualInterventionQueue.length > 0) {
      console.log(
        `📋 ${this.manualInterventionQueue.length} manual interventions pending`
      );
    }
