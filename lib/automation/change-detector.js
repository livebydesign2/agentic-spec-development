...
class ChangeDetector extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      console.error(
        `❌ ChangeDetector initialization failed: ${error.message}`
      );
      return false;
    }
  }
  ...
  async processChange(changePayload) {
    ...
    try {
      ...
      if (changePayload.watcherType === 'yaml') {
        ...
        throw new Error(
          `Unsupported watcher type: ${changePayload.watcherType}`
        );
      }
      ...
      this.stats.averageProcessingTime =
        (this.stats.averageProcessingTime * (this.stats.totalChanges - 1) +
          processingTime) /
        this.stats.totalChanges;
      ...
      if (processingTime > this.performanceTarget) {
        console.warn(
          `⚠️ Change processing took ${processingTime}ms, exceeding ${this.performanceTarget}ms target`
        );
      }
  ...
  async processYamlChange(changePayload) {
    ...
    try {
      if (changeType === 'delete') {
        ...
        if (
          previousState &&
          previousState.parseable &&
          currentState.parseable
        ) {
          analysis.changes = await this.diffYamlStates(
            previousState,
            currentState
          );
          analysis.conflicts = await this.detectYamlConflicts(
            analysis.changes,
            changePayload
          );
        } else if (currentState.parseable) {
          // First time seeing this file or previous state was unparseable
          analysis.changeType = currentState.frontmatter
            ? 'yaml_structure_available'
            : 'yaml_added';
        ...
        analysis.impact = this.assessYamlImpact(
          analysis.changes,
          analysis.semanticChanges
        );
      }
      ...
      if (analysis.conflicts.length > 0) {
        ...
        console.warn(
          ...
          } conflicts detected in ${path.basename(filePath)}`
        );
      }
    ...
    try {
      if (changeType === 'delete') {
        ...
        if (
          ...
          analysis.changes = await this.diffJsonStates(
            previousState,
            currentState
          );
          analysis.conflicts = await this.detectJsonConflicts(
            analysis.changes,
            changePayload
          );
        } else if (currentState.parseable) {
        ...
        // Determine impact level
        analysis.impact = this.assessJsonImpact(
          analysis.changes,
    ...
    for (const change of changes) {
      ...
      if (this.isMetadataField(field)) {
        analysis.semanticChanges.metadataChanges.push({
          ...
          newValue,
          isAutomated:
            field.includes('last_updated') || field.includes('timestamp'),
        });
      }
    }
  }
  ...
  assessYamlImpact(changes, semanticChanges) {
    ...
    if (
      ...
      semanticChanges.statusChange.isWorkflowChange
    ) {
      return 'high';
    }
    ...
    if (
      ...
      semanticChanges.assignmentChange.isHandoff
    ) {
      return 'high';
    }

    if (
      semanticChanges.priorityChange &&
      semanticChanges.priorityChange.priorityEscalation
    ) {
      return 'high';
    }
    ...
    for (const stateType in stateChanges) {
      if (stateChanges[stateType]) {
        significantChangeCount +=
          stateChanges[stateType].significantChanges.length;
      }
    }
  ...
  async detectJsonConflicts(changes, _changePayload) {
    ...
    for (const change of changes) {
      if (
        change.field.includes('current_assignments') &&
        ...
        conflicts.push({
          ...
          severity: 'high',
          suggestedAction:
            'Verify assignment consistency across all state files',
        });
      }
    }
  ...
  isWorkflowStatusChange(oldValue, newValue) {
    ...
    return (
      workflowStatuses.includes(oldValue) && workflowStatuses.includes(newValue)
    );
  }
