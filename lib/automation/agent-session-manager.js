...
class AgentSessionManager extends EventEmitter {
  ...
  async createSession(options) {
    ...
    if (!workspaceId || !agentType) {
      throw new Error(
        'workspaceId and agentType are required for session creation'
      );
    }
    ...
    console.log(
      `[SESSION] Created ${sessionId} for ${agentType} on ${workspaceId}`
    );
  ...
  async processSessionResults(sessionId, results) {
    ...
    try {
      ...
      const processedResults = {
        artifacts: await this.processArtifacts(
          results.artifacts || [],
          session
        ),
        fileChanges: await this.processFileChanges(
          results.fileChanges || [],
          session
        ),
        validationResults: await this.processValidationResults(
          results.validationNeeded || [],
          session
        ),
        nextSteps: await this.processNextSteps(
          results.actionItems || [],
          session
        ),
      ...
      session.performance.metrics.totalExecutionMs =
        Date.now() - session.performance.startTime;
      ...
      throw new Error(
        `Results processing failed for session ${sessionId}: ${error.message}`
      );
    }
  }
  ...
  async completeSession(sessionId, completionData = {}) {
    ...
    console.log(
      `[SESSION] Completed ${sessionId} in ${report.summary.totalDurationMs}ms`
    );
  ...
  generateRecommendations(session) {
    ...
    if (session.performance.metrics.totalExecutionMs > 300000) {
      recommendations.push(
        'Consider breaking down large tasks for better performance'
      );
    }
    ...
    if (
      session.results.validationResults &&
      this.countPassedValidations(session.results.validationResults) === 0
    ) {
