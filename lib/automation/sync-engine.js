...
class SyncEngine extends EventEmitter {
  ...
  async syncYamlToJson(specId, filePath, changes, options = {}) {
    ...
    try {
      ...
      console.log(
        `✅ YAML→JSON sync completed for ${specId} (${syncOperation.duration}ms)`
      );
      ...
      return {
        ...
        withinPerformanceTarget:
          syncOperation.duration <= this.performanceTarget,
      };
    } catch (error) {
  ...
  async syncJsonToYaml(stateChanges, stateType, options = {}) {
    ...
    try {
      console.log(
        `🔄 Starting JSON→YAML sync for ${stateType} (${operationId})`
      );
      syncOperation.state = this.syncStates.IN_PROGRESS;

      // Determine which specs are affected by JSON changes
      const affectedSpecs = await this.identifyAffectedSpecs(
        stateChanges,
        stateType
      );
      ...
      if (affectedSpecs.length === 0) {
        console.log(
          `ℹ️ No specs affected by ${stateType} changes, sync completed`
        );
        syncOperation.state = this.syncStates.COMPLETED;
      ...
      for (const specId of affectedSpecs) {
        ...
        if (!specSyncResult.success) {
          syncOperation.rollbackRequired = true;
          throw new Error(
            `YAML sync failed for ${specId}: ${specSyncResult.error}`
          );
        }
      }
      ...
      console.log(
        `✅ JSON→YAML sync completed for ${stateType} (${syncOperation.duration}ms, ${affectedSpecs.length} specs)`
      );
      ...
      console.error(
        `❌ JSON→YAML sync failed for ${stateType}: ${error.message}`
      );
  ...
  async applyYamlChangesToJson(specId, frontmatter, changes, syncOperation) {
    try {
      ...
      const assignmentChanges = changes.filter(
        (change) =>
          change.field.includes('assigned_agent') ||
          change.field.includes('tasks')
      );

      for (const assignmentChange of assignmentChanges) {
        if (
          assignmentChange.field.includes('tasks.') &&
          assignmentChange.field.includes('assigned_agent')
        ) {
          ...
          if (taskIdMatch) {
            ...
            if (newAgent && assignmentChange.type !== 'deletion') {
              ...
              const assignmentResult =
                await this.workflowStateManager.assignTask(
                  specId,
                  taskId,
                  newAgent,
      ...
      return {
        success: true,
        appliedChanges: syncOperation.appliedChanges.length,
      };
    } catch (error) {
      ...
      if (stateType === 'assignments' && stateChanges.assignments) {
        for (const change of stateChanges.assignments) {
          if (change.field.includes('completed_at')) {
            ...
            const taskIdMatch = change.field.match(
              /current_assignments\.([^.]+)\.([^.]+)\.completed_at/
            );
            if (taskIdMatch) {
            ...
            const taskIdMatch = change.field.match(
              /current_assignments\.([^.]+)\.([^.]+)\.completion_notes/
            );
            if (taskIdMatch) {
              const taskId = taskIdMatch[2];
  ...
  async handleValidationResult(validationResult, eventMetadata) {
    try {
      ...
      if (
        ...
        const syncRequiredInconsistencies =
          validationResult.inconsistencies.filter(
            (inconsistency) =>
              inconsistency.repairStrategy &&
              !inconsistency.repairStrategy.includes('manual_intervention')
          );

        if (syncRequiredInconsistencies.length > 0) {
          console.log(
            `🔄 Validation found ${syncRequiredInconsistencies.length} inconsistencies requiring sync`
          );
  ...
  async handleChangeAnalysis(changeAnalysis, eventMetadata) {
    try {
      ...
      if (
        ...
        console.log(
          `🔄 Change analysis indicates ${changeAnalysis.analysis.impact} impact, triggering sync`
        );

        if (
          changeAnalysis.watcherType === 'yaml' &&
          changeAnalysis.analysis.semanticChanges
        ) {
          ...
          changeAnalysis.watcherType === 'json' &&
  ...
  async rollbackSyncOperation(syncOperation) {
    ...
    try {
      ...
      console.error(
        `❌ Rollback failed for operation ${syncOperation.id}: ${rollbackError.message}`
      );
  ...
  updateSyncMetrics(syncType, success, duration) {
    ...
    this.metrics.averageSyncTime =
      (this.metrics.averageSyncTime * (this.metrics.totalSyncOperations - 1) +
        duration) /
      this.metrics.totalSyncOperations;
    ...
    if (duration > this.performanceTarget) {
      console.warn(
        `⚠️ Sync operation took ${duration}ms, exceeding ${this.performanceTarget}ms target`
      );
    }
  }
  ...
  async identifyAffectedSpecs(stateChanges, stateType) {
    ...
    if (stateType === 'assignments' && stateChanges.assignments) {
      for (const change of stateChanges.assignments) {
        const specIdMatch = change.field.match(
          /current_assignments\.([^.]+)\./
        );
        if (specIdMatch) {
  ...
  async shutdown() {
    ...
    if (this.activeSyncOperations.size > 0) {
      console.log(
        `⏳ Waiting for ${this.activeSyncOperations.size} active sync operations to complete...`
      );
      ...
      while (
        this.activeSyncOperations.size > 0 &&
      ...
      if (this.activeSyncOperations.size > 0) {
        console.warn(
          `⚠️ ${this.activeSyncOperations.size} sync operations did not complete before shutdown`
        );
      }
    }
