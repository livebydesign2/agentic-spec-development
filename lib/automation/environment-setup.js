...
class EnvironmentSetup {
  ...
  async setupEnvironment(workspaceConfig, agentType) {
    ...
    try {
      ...
      console.log(
        `[ENVIRONMENT] Setup ${results.success ? 'successful' : 'failed'} for ${
          workspaceConfig.metadata.workspaceId
        } in ${results.performance.setupTimeMs}ms`
      );

      return results;
    } catch (error) {
      results.success = false;
      results.validation.issues.push(
    ...
    try {
      ...
      if (!results.environment.node.compatible) {
        results.validation.warnings.push(
          `Node.js version ${nodeVersion} may not be compatible. Recommended: 16.x or higher`
        );
      }
      ...
      results.validation.issues.push(
        `Node.js environment validation failed: ${error.message}`
      );
    }
    ...
    try {
      ...
      if (await this.fileExists(packageJsonPath)) {
        ...
        if (await this.fileExists(nodeModulesPath)) {
          results.dependencies.installed = Object.keys(
            packageJson.dependencies || {}
          );
        } else {
          results.dependencies.missing = Object.keys(
            packageJson.dependencies || {}
          );
          results.validation.issues.push(
            'Project dependencies not installed - run npm install'
          );
        }

        // Check for dev dependencies
        results.dependencies.optional = Object.keys(
  ...
  async setupAgentTools(agentType, results) {
    ...
    for (const tool of agentTools) {
      try {
        ...
        results.validation.issues.push(
          `Tool validation failed for ${tool}: ${error.message}`
        );
      }
    }
  ...
  async validateGitEnvironment(workspaceConfig, results) {
    try {
      ...
      if (await this.fileExists(gitDir)) {
        ...
        results.environment.git.cleanWorkingDirectory =
  ...
  async setupEnvironmentVariables(workspaceConfig, agentType, results) {
    try {
      ...
      results.validation.warnings.push(
        `Environment variables setup failed: ${error.message}`
      );
    }
  }
  ...
  async runFinalValidation(results) {
    ...
    try {
      ...
      if (!criticalPassed) {
        results.validation.issues.push(
          'Critical environment requirements not met'
        );
      }

      // Overall validation status
      results.validation.passed =
        criticalPassed && results.validation.issues.length === 0;
  ...
  async installDependencies(projectRoot, results) {
    ...
    try {
      ...
      console.warn(
        `[ENVIRONMENT] Failed to install dependencies: ${error.message}`
      );
    }
  ...
  countAvailableTools(tools) {
    const available = Object.values(tools).filter(
      (tool) => tool.available
    ).length;
