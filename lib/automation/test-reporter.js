...
const __path = require('path'); // eslint-disable-line no-unused-vars
...
class TestReporter extends EventEmitter {
  ...
  normalizeTestResult(testResult) {
    ...
    if (normalized.tests.total > 0 && normalized.executionTime > 0) {
      normalized.performance.averageTestTime =
        normalized.executionTime / normalized.tests.total;
    }
  ...
  addToHistory(normalizedResult) {
    ...
    if (normalizedResult.coverage) {
      ...
      if (this.coverageHistory.length > this.options.historySize) {
        this.coverageHistory = this.coverageHistory.slice(
  ...
  analyzeTrends() {
    ...
    const analysis = {
      ...
      successRate: this.analyzeTrendMetric(
        recent,
        older,
        'tests',
        'passed',
    ...
    if (this.coverageHistory.length >= 2) {
      ...
      if (recentCoverage.length > 0 && olderCoverage.length > 0) {
        const recentAvg =
          recentCoverage.reduce((sum, item) => sum + item.percentage, 0) /
          recentCoverage.length;
        const olderAvg =
          olderCoverage.reduce((sum, item) => sum + item.percentage, 0) /
          olderCoverage.length;
  ...
  analyzeTrendMetric(recent, older, path, subPath = null, calculator = null) {
    ...
    const recentAvg =
      recent.reduce((sum, item) => sum + getValue(item), 0) / recent.length;
    const olderAvg =
      older.reduce((sum, item) => sum + getValue(item), 0) / older.length;
  ...
  analyzeFlakyTests(testResult) {
    ...
    if (
      testResult.metadata.flakyTestsDetected > 0 ||
      testResult.metadata.retryApplied
    ) {
    ...
    analysis.historical = Array.from(this.flakyTestRegistry.entries()).map(
      ([name, data]) => ({
        name,
  ...
  analyzePerformance(testResult) {
    ...
    if (testResult.executionTime <= this.options.performanceThreshold * 0.5) {
      ...
      this.options.performanceThreshold * 1.5
    ) {
    ...
    if (testResult.performance.averageTestTime > 1000) {
      // > 1 second per test
  ...
  generateInsights(testResult, analyses) {
    ...
    // Generate summary
    const successRate =
      testResult.tests.total > 0
        ? (testResult.tests.passed / testResult.tests.total) * 100
        : 0;
    ...
    if (analyses.performance) {
      if (
        analyses.performance.status === 'slow' ||
        analyses.performance.status === 'very_slow'
      ) {
    ...
    if (analyses.flaky && analyses.flaky.detected.length > 0) {
      insights.warnings.push(
        `${analyses.flaky.detected.length} flaky tests detected`
      );
    ...
    if (analyses.trends) {
      if (
        analyses.trends.successRate &&
        analyses.trends.successRate.trend === 'declining'
      ) {
      ...
      if (
        analyses.trends.coverage &&
        analyses.trends.coverage.trend === 'declining'
      ) {
  ...
  generateSummaryReport(data) {
    const { testResult, insights, coverage, _performance } = data; // eslint-disable-line no-unused-vars
  ...
  updateStatistics(testResult) {
    ...
    if (testResult.coverage && testResult.coverage.percentage) {
      this.statistics.averageCoverage =
        (this.statistics.averageCoverage * (this.statistics.totalReports - 1) +
  ...
  getStatistics() {
    return {
      ...
      flakyTests: {
        ...
        active: Array.from(this.flakyTestRegistry.values()).filter(
          (t) =>
            new Date(t.lastSeen) >
