...
class StateValidator extends EventEmitter {
  constructor(configManager = null, workflowStateManager = null) {
    ...
    this.validationRules = {
      ...
      valid_statuses: ['active', 'backlog', 'done', 'blocked', 'in_progress', 'ready'],
      valid_types: ['FEAT', 'BUG', 'SPIKE', 'MAINT', 'RELEASE', 'TASK'],
      valid_priorities: ['P0', 'P1', 'P2', 'P3'],
      valid_agents: ['software-architect', 'senior-developer', 'frontend-specialist', 'backend-specialist', 'qa-specialist', 'devops-specialist']
    };
  ...
  async initialize() {
    try {
      ...
      console.error(`❌ StateValidator initialization failed: ${error.message}`);
  ...
  async validateChangeEvent(changePayload) {
    ...
    try {
      ...
      if (changePayload.watcherType === 'yaml') {
        ...
        throw new Error(`Unsupported change type: ${changePayload.watcherType}`);
      }
      ...
      if (validationTime > this.performanceTarget) {
        console.warn(`⚠️ State validation took ${validationTime}ms, exceeding ${this.performanceTarget}ms target`);
      }
  ...
  async validateYamlChange(changePayload) {
    ...
    try {
      ...
      validationResult.validationDetails.yamlStructure = {
        ...
        hasRequiredFields: yamlState.parseable ? this.hasRequiredYamlFields(yamlState.frontmatter) : false,
      ...
      const structureValidation = await this.validateYamlStructure(yamlState.frontmatter, filePath);
      if (!structureValidation.isValid) {
        validationResult.inconsistencies.push(...structureValidation.inconsistencies);
      }
      ...
      if (analysis && this.affectsCriticalFields(analysis.semanticChanges)) {
        const consistencyResult = await this.validateYamlJsonConsistency(specId, yamlState.frontmatter);
        validationResult.validationDetails.consistencyCheck = consistencyResult;

        if (consistencyResult.inconsistencies.length > 0) {
          validationResult.inconsistencies.push(...consistencyResult.inconsistencies);

          // Attempt automated repairs for simple inconsistencies
          const repairResult = await this.attemptAutomatedRepairs(consistencyResult.inconsistencies, specId, yamlState.frontmatter);
  ...
  async validateJsonChange(changePayload) {
    ...
    try {
      ...
      validationResult.validationDetails.jsonStructure = {
        ...
        hasExpectedStructure: jsonState.parseable ? this.hasExpectedJsonStructure(jsonState.data, stateType) : false,
      ...
      const structureValidation = await this.validateJsonStructure(jsonState.data, stateType, filePath);
      if (!structureValidation.isValid) {
        validationResult.inconsistencies.push(...structureValidation.inconsistencies);
      }
      ...
      if (['assignments', 'progress'].includes(stateType)) {
        const consistencyResult = await this.validateJsonYamlConsistency(jsonState.data, stateType);
        validationResult.validationDetails.consistencyCheck = consistencyResult;
        ...
        if (consistencyResult.inconsistencies.length > 0) {
          ...
          // Attempt automated repairs for simple inconsistencies
          const repairResult = await this.attemptJsonStateRepairs(consistencyResult.inconsistencies, jsonState.data, stateType);
          validationResult.repairsPerformed = repairResult.repairsPerformed;
          validationResult.conflicts.push(...repairResult.conflicts);
        }
      }
  ...
  async validateYamlStructure(frontmatter, filePath) {
    ...
    if (frontmatter.status && !this.validationRules.valid_statuses.includes(frontmatter.status)) {
      validation.inconsistencies.push({
        type: 'invalid_status',
        severity: 'medium',
        field: 'status',
        value: frontmatter.status,
        description: `Invalid status: ${frontmatter.status}. Valid values: ${this.validationRules.valid_statuses.join(', ')}`,
        filePath,
        repairStrategy: 'suggest_valid_values'
      });
    }

    if (frontmatter.type && !this.validationRules.valid_types.includes(frontmatter.type)) {
      validation.inconsistencies.push({
        type: 'invalid_type',
        severity: 'medium',
        field: 'type',
        value: frontmatter.type,
        description: `Invalid type: ${frontmatter.type}. Valid values: ${this.validationRules.valid_types.join(', ')}`,
        filePath,
        repairStrategy: 'suggest_valid_values'
      });
    }

    if (frontmatter.priority && !this.validationRules.valid_priorities.includes(frontmatter.priority)) {
      validation.inconsistencies.push({
        type: 'invalid_priority',
        severity: 'low',
        field: 'priority',
        value: frontmatter.priority,
        description: `Invalid priority: ${frontmatter.priority}. Valid values: ${this.validationRules.valid_priorities.join(', ')}`,
        filePath,
        repairStrategy: 'suggest_valid_values'
      });
    }
    ...
    if (frontmatter.tasks && Array.isArray(frontmatter.tasks)) {
      for (let i = 0; i < frontmatter.tasks.length; i++) {
        ...
        if (task.assigned_agent && !this.validationRules.valid_agents.includes(task.assigned_agent)) {
          validation.inconsistencies.push({
            type: 'invalid_agent',
            severity: 'medium',
            field: `tasks[${i}].assigned_agent`,
            value: task.assigned_agent,
            description: `Invalid agent: ${task.assigned_agent}. Valid values: ${this.validationRules.valid_agents.join(', ')}`,
            filePath,
            repairStrategy: 'suggest_valid_values'
          });
        }
  ...
  async validateJsonStructure(data, stateType, filePath) {
    ...
    switch (stateType) {
      case 'assignments':
        if (!data.current_assignments || typeof data.current_assignments !== 'object') {
          validation.inconsistencies.push({
      ...
      if (frontmatter.tasks && Array.isArray(frontmatter.tasks)) {
        for (const task of frontmatter.tasks) {
          if (task.assigned_agent && task.status === 'in_progress') {
            const jsonAssignment = assignments?.current_assignments?.[specId]?.[task.id];
  ...
  async validateJsonYamlConsistency(jsonData, stateType) {
    ...
    try {
      if (stateType === 'assignments' && jsonData.current_assignments) {
        ...
        for (const [specId, specAssignments] of Object.entries(jsonData.current_assignments)) {
          ...
          if (yamlState?.frontmatter?.tasks) {
            for (const [taskId, assignment] of Object.entries(specAssignments)) {
              const yamlTask = yamlState.frontmatter.tasks.find(t => t.id === taskId);
              ...
              } else if (yamlTask.assigned_agent !== assignment.assigned_agent) {
                consistencyResult.inconsistencies.push({
                  type: 'agent_assignment_mismatch',
                  severity: 'high',
                  specId,
  ...
  async attemptAutomatedRepairs(inconsistencies, specId, frontmatter) {
    ...
    for (const inconsistency of inconsistencies) {
      try {
        if (inconsistency.repairStrategy === 'create_json_assignment' && this.workflowStateManager) {
          // Create missing JSON assignment
          const task = frontmatter.tasks.find(t => t.id === inconsistency.taskId);
          if (task) {
          ...
          repairResult.conflicts.push({
            ...
            suggestedAction: 'Review and manually resolve assignment discrepancy'
          });
        } else if (inconsistency.repairStrategy === 'recalculate_progress' && this.workflowStateManager) {
          ...
          repairResult.repairsPerformed.push({
            ...
            specId,
            description: 'Recalculated progress statistics from current task states'
          });
        }
      } catch (repairError) {
        repairResult.conflicts.push({
  ...
  async attemptJsonStateRepairs(inconsistencies, jsonData, stateType) {
    ...
    for (const inconsistency of inconsistencies) {
      try {
        ...
        } else if (inconsistency.repairStrategy === 'remove_orphaned_assignment') {
          repairResult.conflicts.push({
            ...
            suggestedAction: 'Remove orphaned assignment or create corresponding YAML task'
          });
        }
      } catch (repairError) {
        repairResult.conflicts.push({
  ...
  hasRequiredYamlFields(frontmatter) {
    return this.validationRules.required_yaml_fields.every(field => 
