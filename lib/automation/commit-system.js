...
class CommitSystem extends EventEmitter {
  constructor(gitIntegration, configManager, options = {}) {
    ...
    this.messageTemplates = {
      task_completion: {
        ...
        example:
          'feat(cli): complete TASK-001 - implement git integration system',
      },
      bug_fix: {
        format: 'fix({scope}): {description}',
        example:
          'fix(parser): resolve null pointer exception in feature parsing',
      },
      documentation: {
  ...
  async initialize() {
    try {
      ...
      console.log(
        `✅ CommitSystem initialized - detected hooks: ${
          hooksDetected.join(', ') || 'none'
        }`
      );
  ...
  async createCommit(options = {}) {
    ...
    try {
      ...
      if (!commitMessage) {
        const messageResult = this.generateCommitMessage(
          template,
          templateData
        );
      ...
      while (attempt <= maxAttempts) {
        ...
        if (
          this.isPreCommitHookFailure(commitResult) &&
          this.options.enableHookHandling
        ) {
          const hookHandlingResult = await this.handlePreCommitHookFailure(
            commitResult,
            attempt
          );
      ...
      if (result.success) {
        console.log(
          `✅ Commit created successfully: ${
            result.commitHash?.substring(0, 8) || 'unknown'
          }`
        );
  ...
  async executeCommit(message, options = {}) {
    try {
      ...
      if (commitResult.success) {
        ...
        const verificationResult = await this.verifyCommit(
  ...
  async handlePreCommitHookFailure(commitResult, attempt) {
    try {
      ...
      const analysis = this.analyzeHookFailure(
        commitResult.output || commitResult.error
      );
      ...
      if (analysis.type === 'formatting') {
        ...
        console.log(
          '⚠️  Linting errors detected - manual intervention may be required'
        );
        ...
        console.log(
          '⚠️  Test failures detected - manual intervention required'
        );
  ...
  analyzeHookFailure(output) {
    ...
    if (lowerOutput.includes('prettier') || lowerOutput.includes('format')) {
      ...
      lowerOutput.includes('jest') ||
      lowerOutput.includes('mocha')
    ) {
      ...
      lowerOutput.includes('husky') ||
      lowerOutput.includes('lint-staged')
    ) {
  ...
  async createRollbackPoint() {
    try {
      ...
      const rollbackPoint = {
        ...
        stagedFiles: statusResult.success
          ? statusResult.detailedFiles || []
          : [],
      };
    ...
    try {
      ...
      if (rollbackPoint.stagedFiles.length > 0) {
        ...
        if (filesToRestore.length > 0) {
          const restoreResult = await this.gitIntegration.stageFiles(
            filesToRestore
          );
          if (!restoreResult.success) {
            console.warn(
              `⚠️  Failed to restore staged files: ${restoreResult.error}`
            );
          }
        }
      }
  ...
  displayCommitFailureHelp(result) {
    ...
    if (result.session && result.session.results.length > 0) {
      const lastResult =
        result.session.results[result.session.results.length - 1];
  ...
  updateMetrics(success, executionTime, _result) {
    ...
    this.metrics.averageCommitTime =
      (this.metrics.averageCommitTime * (this.metrics.totalCommits - 1) +
        executionTime) /
      this.metrics.totalCommits;
