...
class WorkspaceManager {
  ...
  async setupWorkspace(options) {
    ...
    if (!specId || !taskId || !agentType) {
      throw new Error('specId, taskId, and agentType are required for workspace setup');
    }
    ...
    try {
      ...
      console.log(`[WORKSPACE] Setup complete for ${workspaceId} in ${setupTime}ms`);

      return config;
    } catch (error) {
      const setupTime = Date.now() - startTime;
      throw new Error(`Workspace setup failed for ${workspaceId} after ${setupTime}ms: ${error.message}`);
    }
  }
  ...
  async createWorkspaceConfiguration(options) {
    const { workspaceId, specId, taskId, agentType, taskContext, _baseContext } = options;
    ...
    if (taskContext?.relatedFiles) {
      for (const [category, files] of Object.entries(taskContext.relatedFiles)) {
  ...
  async initializeFilePaths(config) {
    ...
    try {
      ...
      for (const file of config.files.primary) {
        if (!file.exists) {
          config.environment.validation.issues.push(`Primary file not found: ${file.relativePath}`);
        }
      }
    ...
    try {
      ...
      for (const tool of nodeTools) {
        ...
        if (available) {
          ...
          config.environment.validation.issues.push(`Required tool not available: ${tool}`);
        }
      }
      ...
      for (const tool of devTools) {
        ...
        if (available) {
          ...
          config.environment.validation.warnings.push(`Optional tool not available: ${tool}`);
        }
      }
      ...
      if (await this.fileExists(packageJsonPath)) {
        ...
        if (!await this.fileExists(nodeModulesPath)) {
          config.environment.validation.warnings.push('node_modules not found - may need npm install');
        }

        config.dependencies.required = Object.keys(packageJson.dependencies || {});
        config.dependencies.optional = Object.keys(packageJson.devDependencies || {});
      }
      ...
      config.environment.validation.passed = config.environment.validation.issues.length === 0;
      config.performance.validationTimeMs = Date.now() - startTime;

    } catch (error) {
      config.environment.validation.issues.push(`Environment validation failed: ${error.message}`);
  ...
  async setupAgentResources(config, agentType) {
    ...
    if (agentResources.workspaceFiles) {
      for (const [filename, content] of Object.entries(agentResources.workspaceFiles)) {
  ...
  async cleanupWorkspace(workspaceId, options = {}) {
    ...
    try {
      ...
      console.log(`[WORKSPACE] Cleaned ${workspaceId}: ${results.cleaned.length} categories cleaned`);
  ...
  async validateFilePermissions(config) {
    try {
      ...
      config.environment.validation.issues.push(`Insufficient write permissions in project directory: ${error.message}`);
    }
    ...
    try {
      ...
      config.environment.validation.issues.push(`Insufficient write permissions in workspace directory: ${error.message}`);
    }
  }
  ...
  listActiveWorkspaces() {
    return Array.from(this.activeWorkspaces.entries()).map(([id, workspace]) => ({
      workspaceId: id,
  ...
  async cleanupAllInactive() {
    ...
    const inactive = Array.from(this.activeWorkspaces.entries())
      .filter(([_, workspace]) => workspace.status === 'completed' || workspace.status === 'failed');

    for (const [workspaceId, __] of inactive) {
      try {
        await this.cleanupWorkspace(workspaceId);
        results.cleaned++;
      } catch (error) {
        results.errors.push(`Failed to cleanup ${workspaceId}: ${error.message}`);
      }
    }
