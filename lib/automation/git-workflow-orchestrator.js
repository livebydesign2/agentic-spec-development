...
class GitWorkflowOrchestrator extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      console.error(
        `❌ GitWorkflowOrchestrator initialization failed: ${error.message}`
      );
      return false;
    }
  }
  ...
  async initializeSubsystems() {
    ...
    if (this.options.enableCommitAutomation) {
      this.commitSystem = new CommitSystem(
        this.gitIntegration,
        this.configManager,
        this.options
      );
  ...
  async executeWorkflow(options = {}) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (
          rollbackOnFailure &&
          this.currentWorkflow.rollbackPoints.length > 0
        ) {
  ...
  async executeWorkflowStages(stageOptions) {
    ...
    try {
      for (const [stageName, stageConfig] of Object.entries(
        ...
        if (
          (stageName === 'executeLinting' && stageOptions.skipLinting) ||
        ...
        if (stageResult.success) {
          console.log(
            `✅ ${stageConfig.name} completed (${stageExecutionTime}ms)`
          );
        } else {
          ...
          if (!stageConfig.required) {
            console.log(
              `⚠️  Continuing workflow despite ${stageConfig.name} failure (non-required stage)`
            );
          }
        }
      }
  ...
  async executeTestingStage(_options) {
    try {
      ...
      if (this.testReporter && testingResult.success) {
        const reportResult = await this.testReporter.processTestResults(
          testingResult,
          {
            reportType: 'summary',
            analyzeTrends: true,
  ...
  async createRollbackPoint(stageName) {
    try {
      ...
      console.warn(
        `⚠️  Failed to create rollback point for ${stageName}: ${error.message}`
      );
      return null;
    }
  }
  ...
  async executeRollback() {
    try {
      ...
      const rollbackPoint =
        this.currentWorkflow.rollbackPoints[
          this.currentWorkflow.rollbackPoints.length - 1
        ];
  ...
  displayWorkflowSummary(result) {
    ...
    if (
      result.results.executeTesting &&
      result.results.executeTesting.testResults
    ) {
      const tests = result.results.executeTesting.testResults;
      console.log(
        `\n🧪 Tests: ${tests.passedTests || 0} passed, ${
          tests.failedTests?.length || 0
        } failed`
      );
    }
  }
  ...
  updateMetrics(success, executionTime, result) {
    ...
    this.metrics.averageWorkflowTime =
      (this.metrics.averageWorkflowTime * (this.metrics.totalWorkflows - 1) +
        executionTime) /
      this.metrics.totalWorkflows;
  ...
  async shutdown() {
    try {
      ...
      if (this.gitIntegration)
        shutdownPromises.push(this.gitIntegration.shutdown());
      if (this.fileTracker) shutdownPromises.push(this.fileTracker.shutdown());
      if (this.lintingSystem)
        shutdownPromises.push(this.lintingSystem.shutdown());
      if (this.testingSystem)
        shutdownPromises.push(this.testingSystem.shutdown());
      if (this.commitSystem)
        shutdownPromises.push(this.commitSystem.shutdown());
      ...
      console.error(
        `❌ GitWorkflowOrchestrator shutdown failed: ${error.message}`
      );
    }
  }
}
