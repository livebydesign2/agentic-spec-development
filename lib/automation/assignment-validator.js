...
class AssignmentValidator {
  ...
  async validateAssignment(assignment, options = {}) {
    ...
    try {
      ...
      validation.validationDetails.taskAvailability = await this.validateTaskAvailability(
        specId,
        taskId
      );
      ...
      if (!validation.validationDetails.taskAvailability.isValid) {
        ...
        validation.violations.push(...validation.validationDetails.taskAvailability.violations);
      }

      // 2. Agent Capability Validation
      validation.validationDetails.agentCapability = await this.validateAgentCapability(
        agentType,
        specId,
        taskId
      );
      ...
      if (!validation.validationDetails.agentCapability.isValid) {
        validation.isValid = false;
        validation.violations.push(...validation.validationDetails.agentCapability.violations);
      }
      validation.confidence *= validation.validationDetails.agentCapability.confidenceMultiplier || 1.0;

      // 3. Dependency Block Validation
      validation.validationDetails.dependencyCheck = await this.validateDependencies(
        specId,
        taskId
      );
      ...
      if (!validation.validationDetails.dependencyCheck.isValid) {
        validation.isValid = false;
        validation.violations.push(...validation.validationDetails.dependencyCheck.violations);
      }

      // 4. Workload Balance Validation
      if (options.checkWorkload) {
        validation.validationDetails.workloadBalance = await this.validateWorkloadBalance(
          agentType,
        ...
        if (!validation.validationDetails.workloadBalance.isValid) {
          validation.isValid = false;
          validation.violations.push(...validation.validationDetails.workloadBalance.violations);
        }
        validation.warnings.push(...validation.validationDetails.workloadBalance.warnings);
      }

      // 5. Business Rules Validation
      validation.validationDetails.businessRules = await this.validateBusinessRules(
        assignment,
        options
      );
      ...
      if (!validation.validationDetails.businessRules.isValid) {
        validation.isValid = false;
        validation.violations.push(...validation.validationDetails.businessRules.violations);
      }
      ...
      validation.confidence = Math.max(0.1, Math.min(1.0, validation.confidence));
  ...
  async validateTaskAvailability(specId, taskId) {
    ...
    try {
      ...
      validation.isAvailable = availableStatuses.includes(validation.currentStatus);
      ...
      const currentAssignments = await this.workflowStateManager.getCurrentAssignments();
      const existingAssignment = currentAssignments.current_assignments.find(a =>
      ...
      validation.isValid = false;
      validation.violations.push(`Task availability check failed: ${error.message}`);
    }
  ...
  async validateAgentCapability(agentType, specId, taskId) {
    ...
    try {
      ...
      if (!task) {
        validation.isValid = false;
        validation.violations.push(`Task ${specId}:${taskId} not found for capability validation`);
        return validation;
      }

      // Use TaskRouter's built-in capability validation
      const canHandle = this.taskRouter.validateAgentCapability(task, agentType);
      ...
      const skillValidation = await constraintEngine.validateSkillConstraints(task, agentType);
      ...
      validation.isValid = false;
      validation.violations.push(`Agent capability validation failed: ${error.message}`);
  ...
  async validateDependencies(specId, taskId) {
    ...
    try {
      ...
      if (!task) {
        validation.isValid = false;
        validation.violations.push(`Task ${specId}:${taskId} not found for dependency validation`);
        return validation;
      }
      ...
      if (isBlocked) {
        ...
        const dependencyChain = await this.taskRouter.getDependencyChain(taskId);
      ...
      validation.isValid = false;
      validation.violations.push(`Dependency validation failed: ${error.message}`);
    }
  ...
  async validateWorkloadBalance(agentType, constraints = {}) {
    ...
    try {
      ...
      validation.projectedWorkload = validation.currentWorkload + estimatedHours;

      // Validate workload constraints
      const workloadValidation = await constraintEngine.validateWorkloadConstraints(
  ...
  async validateBusinessRules(assignment, options = {}) {
    ...
    try {
      ...
      const currentAssignments = await this.workflowStateManager.getCurrentAssignments();
      const agentAssignments = currentAssignments.current_assignments.filter(a =>
        a.assigned_agent === agentType && a.status === 'in_progress'
      );
      ...
      validation.violations.push(`Business rules validation failed: ${error.message}`);
    }
  ...
  getActionableErrors(validation) {
    ...
    for (const violation of validation.violations) {
      if (violation.includes('not found')) {
        actionableErrors.push(`${violation}. Check task ID and spec ID are correct.`);
      } else if (violation.includes('already assigned')) {
        actionableErrors.push(`${violation}. Use 'asd tasks --agent ${validation.agentType}' to see current assignments.`);
      } else if (violation.includes('blocked by')) {
        actionableErrors.push(`${violation}. Complete dependencies first or check dependency status.`);
      } else if (violation.includes('cannot handle')) {
        actionableErrors.push(`${violation}. Check agent capabilities or assign to appropriate agent type.`);
      } else if (violation.includes('workload') || violation.includes('capacity')) {
        actionableErrors.push(`${violation}. Consider load balancing or increasing agent capacity.`);
      } else if (violation.includes('P0') && violation.includes('confirmation')) {
        actionableErrors.push(`${violation}. Use --confirm-critical flag for critical task assignment.`);
      } else {
