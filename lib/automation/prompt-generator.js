...
class PromptGenerator {
  ...
  async generateTaskPrompt(options) {
    ...
    if (!agentType || !taskContext) {
      throw new Error(
        'agentType and taskContext are required for prompt generation'
      );
    }
    ...
    try {
      ...
      const generatedPrompt = await this.renderPrompt(
        template,
        prioritizedContext
      );

      // Step 5: Validate and optimize prompt
      const validatedPrompt = await this.validateAndOptimizePrompt(
        generatedPrompt,
        agentType
      );
      ...
      throw new Error(
        `Prompt generation failed after ${totalTime}ms: ${error.message}`
      );
    }
  }
  ...
  async loadPromptTemplate(agentType) {
    ...
    try {
      ...
      throw new Error(
        `Failed to load prompt template for ${agentType}: ${error.message}`
      );
    }
  }
  ...
  async preparePromptContext(options) {
    ...
    const promptContext = {
      ...
      specification: {
        title:
          taskContext.taskSpecific.specification?.frontmatter?.title ||
          'Unknown Specification',
        priority:
          taskContext.taskSpecific.specification?.frontmatter?.priority || 'P2',
        status: taskContext.taskSpecific.specification?.status || 'unknown',
        description: this.extractSpecDescription(
          taskContext.taskSpecific.specification?.content || ''
        ),
        requirements:
          taskContext.taskSpecific.specification?.requirements || [],
  ...
  async prioritizeContext(promptContext, agentType) {
    ...
    prioritized.resources = this.filterResourcesByRelevance(
    ...
    prioritized.specification.successCriteria =
      promptContext.specification.successCriteria.slice(0, 8);

    // Filter constraints by relevance
    prioritized.project.constraints = promptContext.project.constraints
      .filter((constraint) =>
        this.isConstraintRelevant(constraint, agentType, promptContext.task)
      )
      .slice(0, 10);
  ...
  async validateAndOptimizePrompt(generatedPrompt, agentType) {
    ...
    if (generatedPrompt.prompt.length > this.maxPromptLength) {
      validation.issues.push(
        `Prompt too long: ${generatedPrompt.prompt.length} characters (max: ${this.maxPromptLength})`
      );

      // Attempt to truncate intelligently
      const truncated = await this.truncatePromptIntelligently(
    ...
    const optimized = await this.optimizeForAgent(
      generatedPrompt.prompt,
      agentType
    );
...
Complete all checklist items and update the specification when done.`;
  ...
  async getAgentCapabilities(agentType) {
    try {
      ...
      console.warn(
        `Failed to load capabilities for ${agentType}: ${error.message}`
      );
    }
  ...
  async getAgentSpecializations(agentType) {
    try {
      ...
      console.warn(
        `Failed to load specializations for ${agentType}: ${error.message}`
      );
    }
  ...
  extractValidationRequirements(taskContext) {
    ...
    if (taskContent.includes('test'))
      requirements.push('comprehensive testing');
    if (taskContent.includes('performance'))
      requirements.push('performance validation');
    if (taskContent.includes('integration'))
  ...
  filterResourcesByRelevance(resources, _agentType, _priorities) {
    const filtered = {};
  ...
  isConstraintRelevant(constraint, agentType, task) {
    ...
    return (
      agentKeywords.some((keyword) => constraintText.includes(keyword)) ||
  ...
  async truncatePromptIntelligently(prompt, _agentType) {
    ...
    for (const section of sections.slice(1)) {
      const sectionName = section.split('\n')[0];
      if (
        !criticalSections.some((critical) => sectionName.includes(critical))
      ) {
        if (currentLength + section.length < targetLength) {
          truncated += '\n## ' + section;
          currentLength += section.length;
  ...
  initializeHelpers() {
    ...
    Handlebars.registerHelper('formatList', function (items, _options) {
