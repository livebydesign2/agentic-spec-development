...
class ContextGatherer {
  ...
  async gatherTaskContext(options) {
    ...
    if (!specId || !taskId) {
      throw new Error(
        'Both specId and taskId are required for task context gathering'
      );
    }
    ...
    try {
      ...
      context.metadata.performance.sections.specification =
        performance.now() - specStart;

      // 2. Extract task details from specification
      const taskStart = performance.now();
      context.taskSpecific.task = await this.extractTaskDetails(
        context.taskSpecific.specification,
        taskId
      );
      context.metadata.performance.sections.taskExtraction =
        performance.now() - taskStart;

      // 3. Gather dependencies and related specifications
      const depsStart = performance.now();
      context.dependencies = await this.gatherDependencies(
        context.taskSpecific.specification
      );
      context.metadata.performance.sections.dependencies =
        performance.now() - depsStart;

      // 4. Find and include related files if requested
      if (includeFiles) {
        const filesStart = performance.now();
        context.relatedFiles = await this.gatherRelatedFiles(
          context.taskSpecific.task,
          agentType
        );
        context.metadata.performance.sections.relatedFiles =
          performance.now() - filesStart;
      }

      // 5. Calculate context validation and relevance
      const validationStart = performance.now();
      await this.validateAndScoreContext(context, agentType);
      context.metadata.performance.sections.validation =
        performance.now() - validationStart;

      // 6. Apply agent-specific filtering
      const filteringStart = performance.now();
      const filteredContext = await this.applyAgentFiltering(
        context,
        agentType
      );
      filteredContext.metadata.performance.sections.filtering =
        performance.now() - filteringStart;

      // Record total performance
      const totalTime = performance.now() - startTime;
      filteredContext.metadata.performance.gatheringTimeMs =
      ...
      const totalTime = performance.now() - startTime;
      throw new Error(
        `Context gathering failed after ${Math.round(totalTime)}ms: ${
          error.message
        }`
      );
    }
  ...
  async loadSpecification(specId) {
    ...
    for (const specPath of possiblePaths) {
      try {
        ...
        console.warn(
          `Failed to load specification from ${specPath}: ${error.message}`
        );
      }
    }
  ...
  async extractTaskDetails(specification, taskId) {
    ...
    if (!match) {
      throw new Error(
        `Task ${taskId} not found in specification ${specification.filePath}`
      );
    }
  ...
  async gatherDependencies(specification) {
    ...
    for (const specPath of allSpecs) {
      try {
        ...
        console.warn(
          `Failed to check dependency relationship for ${specPath}: ${error.message}`
        );
      }
    }
  ...
  async gatherRelatedFiles(task, agentType) {
          ...
          relatedFiles[category].push({
            ...
            content:
              content.length > 5000
                ...
                : content,
            size: content.length,
            relevance: await this.calculateFileRelevance(
              file,
              content,
              task,
              agentType
    ...
    const additionalFiles = await this.findAdditionalRelevantFiles(
      task,
      agentType,
      projectRoot
    );
  ...
  async validateAndScoreContext(context, agentType) {
    ...
    validation.relevanceScore = await this.calculateContextRelevance(
      context,
      agentType
    );

    // Determine if context is sufficient
    validation.isSufficient =
      validation.completeness >= 0.7 &&
      validation.relevanceScore >= this.relevanceThreshold;
  }
  ...
  async applyAgentFiltering(context, agentType) {
    ...
    try {
      ...
      if (filtered.relatedFiles) {
        filtered.relatedFiles = this.filterFilesByAgentType(
      ...
      console.warn(
        `Failed to load agent definition for filtering: ${error.message}`
      );
    }
  ...
  async calculateContextRelevance(context, agentType) {
    ...
    const contentRelevance = await this.calculateContentRelevance(
      context,
      agentType
    );
    ...
    const fileRelevance = this.calculateRelatedFilesRelevance(
      context.relatedFiles,
      agentType
    );
    relevanceScore += fileRelevance * 0.2;
    factors += 0.2; // eslint-disable-line no-unused-vars

    // Factor 4: Dependencies relevance (10% weight)
    const depRelevance = this.calculateDependencyRelevance(
  ...
  extractTaskStatus(taskContent) {
    if (
      taskContent.includes('← READY FOR PICKUP') ||
      taskContent.includes('⏳')
    )
      return 'ready';
    if (taskContent.includes('🔄') || taskContent.includes('In progress'))
      return 'in_progress';
    if (taskContent.includes('✅') || taskContent.includes('Complete'))
      return 'complete';
    if (taskContent.includes('⏸️') || taskContent.includes('BLOCKED'))
      return 'blocked';
    return 'pending';
  }
  ...
  filterFilesByAgentType(relatedFiles, agentType) {
    ...
    if (agentType === 'software-architect') {
      filtered.implementation = filtered.implementation.filter(
        (file) =>
          file.relevance > 0.3 ||
