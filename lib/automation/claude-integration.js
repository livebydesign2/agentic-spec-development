...
class ClaudeIntegration {
  ...
  async sendTaskToAgent(options) {
    ...
    if (!generatedPrompt || !workspaceConfig) {
      throw new Error(
        'Generated prompt and workspace configuration are required'
      );
    }
    ...
    try {
      ...
      const claudeResponse = await this.sendClaudeRequest(
        claudeRequest,
        apiKey,
        requestId
      );
      ...
      const formattedResults = await this.formatAgentResults(
        processedResponse,
        workspaceConfig
      );
      ...
      throw new Error(
        `Claude sub-agent integration failed after ${totalTime}ms: ${error.message}`
      );
    }
  }
  ...
  async sendClaudeRequest(claudeRequest, apiKey, requestId) {
    ...
    if (!requestApiKey) {
      throw new Error(
        'Claude API key not provided and CLAUDE_API_KEY environment variable not set'
      );
    }
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        console.warn(
          `[CLAUDE] Attempt ${attempt}/${this.maxRetries} failed: ${error.message}`
        );
    ...
    throw new Error(
      `Claude API request failed after ${this.maxRetries} attempts: ${lastError.message}`
    );
  }
  ...
  async formatAgentResults(processedResponse, workspaceConfig) {
    const results = {
      ...
      validationNeeded: this.extractValidationRequirements(
  ...
  async validateAgentResponse(response) {
    ...
    const hasCode =
      ...
      response.content.includes('const ') ||
    ...
    validation.isValid =
      validation.score >= 0.5 && validation.issues.length === 0;
        ...
        messages: [
          {
            role: 'user',
            content:
              'Hello, this is a connectivity test. Please respond with "Connected successfully".',
          },
        ],
      };
