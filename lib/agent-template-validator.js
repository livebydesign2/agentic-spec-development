...
class AgentTemplateValidator {
  ...
  async validateAgent(agentFilePath) {
    ...
    try {
      ...
      validation.errors.push(
        `Failed to parse agent definition: ${error.message}`
      );
      validation.isValid = false;
    }
  ...
  validateFieldFormats(agentConfig, validation) {
    ...
    if (agentConfig.agent_type) {
      if (!/^[a-z-]+$/.test(agentConfig.agent_type)) {
        validation.errors.push(
          'agent_type must be lowercase with hyphens only'
        );
        validation.isValid = false;
      }
    }
  ...
  validateWorkflowSteps(agentConfig, validation) {
    if (
      !agentConfig.workflow_steps ||
      !Array.isArray(agentConfig.workflow_steps)
    ) {
      return;
    }
    ...
    agentConfig.workflow_steps.forEach((step, index) => {
      if (!step.step) {
        validation.errors.push(
          `Workflow step ${index + 1} missing 'step' field`
        );
        validation.isValid = false;
      }
      ...
      if (!step.description) {
        validation.errors.push(
          `Workflow step ${index + 1} missing 'description' field`
        );
        validation.isValid = false;
      }
      ...
      if (!step.validation || !Array.isArray(step.validation)) {
        validation.errors.push(
          `Workflow step ${index + 1} missing 'validation' array`
        );
        validation.isValid = false;
      } else if (step.validation.length === 0) {
        validation.warnings.push(
          `Workflow step ${index + 1} has empty validation criteria`
        );
      }
    });
  }
  ...
  validateCapabilities(agentConfig, validation) {
    ...
    agentConfig.capabilities.forEach((capability, index) => {
      ...
      if (vagueWords.some((word) => capability.toLowerCase().includes(word))) {
        validation.warnings.push(
          `Capability ${index + 1} uses vague language: "${capability}"`
        );
      }
      ...
      if (capability.length < 20) {
        validation.warnings.push(
          `Capability ${index + 1} may be too brief: "${capability}"`
        );
      }
      ...
      if (
        !actionVerbs.some((verb) => capability.toLowerCase().startsWith(verb))
      ) {
        validation.suggestions.push(
          `Consider starting capability ${index + 1} with an action verb`
        );
      }
    });
  }
  ...
  validateContextRequirements(agentConfig, validation) {
    if (
      !agentConfig.context_requirements ||
      !Array.isArray(agentConfig.context_requirements)
    ) {
      return;
    }
    ...
    agentConfig.context_requirements.forEach((requirement, index) => {
      ...
      if (!hasStandardPattern) {
        validation.suggestions.push(
          `Context requirement ${
            index + 1
          } could use more standard terminology: "${requirement}"`
        );
      }
      ...
      if (requirement.includes('_') && requirement.includes('-')) {
        validation.warnings.push(
          `Context requirement ${
            index + 1
          } mixes hyphens and underscores: "${requirement}"`
        );
      }
    });
  }
  ...
  validateHandoffChecklist(agentConfig, validation) {
    if (
      !agentConfig.handoff_checklist ||
      !Array.isArray(agentConfig.handoff_checklist)
    ) {
      return;
    }
  ...
  validateConsistency(agentConfig, validation) {
    ...
    if (agentConfig.is_template === true) {
      validation.warnings.push(
        'Agent definition marked as template - remove is_template for production use'
      );
    }
      ...
      agentConfig.specializations.forEach((spec, index) => {
        if (spec.includes(' ')) {
          validation.warnings.push(
            `Specialization ${
              index + 1
            } contains spaces, consider using hyphens: "${spec}"`
          );
        }
      });
    }
  ...
  generateSuggestions(agentConfig, validation) {
    ...
    if (!agentConfig.technology_focus) {
      validation.suggestions.push(
        'Consider adding technology_focus section for better context'
      );
    }
    ...
    if (!agentConfig.quality_standards) {
      validation.suggestions.push(
        'Consider adding quality_standards section for clarity'
      );
    }
    ...
    if (agentConfig.workflow_steps) {
      ...
      const hasConsistentNaming = stepNames.every(
        (name) => name.includes('_') || stepNames.every((n) => !n.includes('_'))
      );

      if (!hasConsistentNaming) {
        validation.suggestions.push(
          'Consider consistent naming convention for workflow steps'
        );
      }
    }
  }
