...
class RequiredFieldsRule extends ValidationRule {
  ...
  canAutoFix(error) {
    ...
    return error.type === 'missing_required_field' && autoFixableFields.includes(error.field);
  }
  ...
  getFixSuggestion(error) {
    if (this.canAutoFix(error)) {
      return [`Add ${error.field}: ${this._getDefaultValue(error.field)} to the frontmatter`];
    }
...
class IDFormatRule extends ValidationRule {
  ...
  async validate(spec, _context) {
    ...
    const supportedTypes = _context.supportedTypes || ['SPEC', 'FEAT', 'BUG', 'SPIKE', 'MAINT', 'RELEASE'];
  ...
  async validate(spec, _context) {
    ...
    if (!validPriorities.includes(spec.priority)) {
      ...
      result.errors.push({
        ...
        message: `Invalid priority: ${spec.priority}. Must be one of: ${validPriorities.join(', ')}`,
        file: spec.filePath,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.match(/^priority:/i)) {
        ...
        let normalized = line
          ...
          .replace(/priority:\s*p([0-3])/i, (match, num) => `priority: P${num}`);
...
class StatusValidationRule extends ValidationRule {
  ...
  async validate(spec, _context) {
    ...
    const validStatuses = _context.statusFolders || ['active', 'backlog', 'done'];
    if (!validStatuses.includes(spec.status)) {
      result.valid = false;
      result.errors.push({
        type: 'invalid_status',
        message: `Invalid status: ${spec.status}. Must be one of: ${validStatuses.join(', ')}`,
        file: spec.filePath,
  ...
  async autoFix(content, error) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.match(/^status:/i)) {
        ...
        let normalized = line
          .replace(/status:\s*(in.progress|in_progress|inprogress)/i, 'status: active')
...
class AgentTypeValidationRule extends ValidationRule {
  ...
  getFixSuggestion(error) {
    if (error.type === 'unknown_agent_type') {
      return [
        `Consider using a known agent type: ${error.knownTypes.slice(0, 5).join(', ')}...`,
  ...
  async validate(specs, _context) {
    ...
    for (const spec of specs) {
      if (spec.dependencies && Array.isArray(spec.dependencies)) {
        const invalidDeps = spec.dependencies.filter(dep => !allSpecIds.has(dep));

        if (invalidDeps.length > 0) {
          result.valid = false;
          result.errors.push({
            type: 'invalid_spec_dependencies',
            message: `Invalid spec dependencies in ${spec.id}: ${invalidDeps.join(', ')}`,
            specId: spec.id,
            file: spec.filePath,
            invalidDeps: invalidDeps,
            severity: this.severity,
...
class TransitionValidationRule extends ValidationRule {
  ...
  async validate(data, _context) {
    ...
    const { specId, fromStatus, toStatus } = data;
  ...
  getFixSuggestion(error) {
    const valid = error.validTransitions.length > 0
