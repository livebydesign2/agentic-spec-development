...
/**
 * Reference Integrity Validation Rule - Validates all inter-spec references (MAINT-003 TASK-004)
 */
class ReferenceIntegrityRule extends ValidationRule {
  constructor() {
    super('reference-integrity', 'consistency', 'error');
  }

  async validate(specs, context) {
    const result = {
      valid: true,
      errors: [],
      warnings: [],
    ...
    const allSpecIds = new Set(specs.map(s => s.id).filter(Boolean));
    ...
    for (const spec of specs) {
      ...
      if (spec.blocking && Array.isArray(spec.blocking)) {
        for (const blockId of spec.blocking) {
          if (!allSpecIds.has(blockId)) {
            result.valid = false;
            result.errors.push({
              type: 'invalid_blocking_reference',
              message: `Invalid blocking reference in ${spec.id}: '${blockId}' does not exist`,
              specId: spec.id,
              file: spec.filePath,
              targetId: blockId,
      ...
      if (spec.related && Array.isArray(spec.related)) {
        for (const relatedId of spec.related) {
          if (!allSpecIds.has(relatedId)) {
            result.valid = false;
            result.errors.push({
              type: 'invalid_related_reference',
              message: `Invalid related reference in ${spec.id}: '${relatedId}' does not exist`,
              specId: spec.id,
              file: spec.filePath,
              targetId: relatedId,
  ...
  getFixSuggestion(error) {
    return [
      `Remove or correct the invalid ${error.referenceType} reference: ${error.targetId}`,
      'Ensure the target specification exists and has the correct ID'
    ];
  }
}
