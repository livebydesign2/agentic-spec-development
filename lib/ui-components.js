class UIComponents {
  ...
  renderProgressBar(term, progressInfo) {
    if (!progressInfo.hasColor) {
      ...
      term(
        `[${progressInfo.filled}${progressInfo.empty}] ${progressInfo.percentage}`
      );
      return;
    }
  ...
  createSideTabbedHeader(currentView, _focusedPanel, _maxWidth) {
    const tabs = [
  ...
  createFeatureListItem(feature, progress, selected = false) {
    ...
    const completedText = feature.completedDate
      ? ` | Completed: ${this.formatDate(feature.completedDate)}`
      : '';
  ...
  createTaskList(tasks, indent = '  ') {
    ...
    return tasks
      .map((task) => {
        ...
        const assignee = task.assigneeRole ? ` [${task.assigneeRole}]` : '';
  ...
  createFooter(focusedPanel) {
    ...
    const focusHint = focusedPanel
      ? ` | Focus: ${focusedPanel.toUpperCase()}`
      : '';
  ...
  createHelpText() {
    return [
      ...
      '  1-3           - Jump directly to panel (1=Main, 2=Overview/Roadmap, 3=Details)',
      '  Tab           - Cycle through panels',
      '  Shift+Tab     - Switch panel tabs (main: Active/Backlog/Done, [2]: Overview/Roadmap, [4]: Current/Next Task)',
