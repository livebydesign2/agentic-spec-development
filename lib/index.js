...
class ASDClient {
  ...
  calculateLayout() {
    ...
    const availableHeightAfterRecommended =
      height - topPanelsY - this.LAYOUT.GAP - this.LAYOUT.FOOTER_HEIGHT - this.LAYOUT.GAP;
  ...
  drawPanelBorders(layout) {
    ...
    const mainColor = this.focusedPanel === 'main' ? 'brightGreen' : 'brightBlue';
    this.drawBox(
      panelCoords.main.x,
      panelCoords.main.y,
    ...
    const sideColor = this.focusedPanel === 'side' ? 'brightGreen' : 'brightBlue';
    this.drawBox(
      panelCoords.side.x,
      panelCoords.side.y,
    ...
    const bottomColor = this.focusedPanel === 'bottom' ? 'brightGreen' : 'brightBlue';
    this.drawBox(
  ...
  async render() {
    ...
    try {
      ...
      this.renderCount++;
      if (this.renderCount % 25 === 0) { // More frequent cleanup
      ...
      if (this.renderCount % 100 === 0) {
        ...
        if (rssInMB > 200) { // If using more than 200MB, force cleanup
          this.performMemoryCleanup();
          if (process.env.DEBUG_MEMORY) {
            console.warn(`High memory usage detected: ${Math.round(rssInMB)}MB, forcing cleanup`);
          }
        }
      }
    } catch (error) {
  ...
  async renderMainPanel() {
    ...
    const maxVisibleFeatures = Math.max(1, Math.floor(contentArea.height / avgLinesPerFeature));
    ...
    this.scrollOffsets.main = Math.max(0, Math.min(scrollOffset, features.length - maxVisibleFeatures));

    // Render visible features
    let lineY = contentArea.y;
    const visibleFeatures = features.slice(this.scrollOffsets.main, this.scrollOffsets.main + maxVisibleFeatures);
    ...
    for (let i = 0; i < visibleFeatures.length; i++) {
      ...
      const featureDisplay = this.ui.createFeatureListItem(feature, progress, isSelected);
      const lines = featureDisplay.hasProgressBar ? featureDisplay.lines : featureDisplay.split('\n');
      ...
      for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        else if (isSelected && this.focusedPanel === 'main' && lineIndex === 0) {
          const lineText = typeof line === 'string' ? line.substring(0, contentArea.width - 2) : line;
      ...
      if (i < visibleFeatures.length - 1 && lineY < contentArea.y + contentArea.height - 1) {
    ...
    if (features.length > maxVisibleFeatures) {
      const scrollIndicator = this.ui.getScrollIndicator(this.scrollOffsets.main, features.length, maxVisibleFeatures);
      if (scrollIndicator) {
        term.moveTo(contentArea.x + contentArea.width - 1, contentArea.y + Math.floor(contentArea.height / 2));
  ...
  async renderSidePanel() {
    ...
    if (currentY < contentArea.height - 3) {
      ...
      const criticalFeatures = allSpecs.filter(f => f.priority === 'P0').slice(0, 3);
  ...
  async renderBottomPanel() {
    ...
    term.brightCyan(`${statusIcon} ${selectedFeature.id}: ${selectedFeature.title}`);

    // Progress line
    if (lineY < contentArea.y + contentArea.height) {
      term.moveTo(contentArea.x, lineY++);
      term(`Progress: ${progress.completed}/${progress.total} subtasks complete`);
    }
    ...
    if (
      ...
      const taskList = this.ui.createTaskList(selectedFeature.tasks.slice(0, contentArea.height - 5));
  ...
  async renderRecommendedPanel() {
    ...
    if (this.recommendedView === 'current') {
      if (this.currentTask) {
        ...
        const titleLines = this.ui.wrapText(this.currentTask.title, contentArea.maxWidth, 2);
      ...
      if (nextTasks.length > 0) {
        ...
        const tasksToShow = nextTasks.slice(0, Math.min(4, Math.floor((contentArea.height - 2) / 2)));
        ...
        if (nextTasks.length > tasksToShow.length && currentY < contentArea.height) {
          term.moveTo(contentArea.x, contentArea.y + currentY++);
  ...
  performMemoryCleanup() {
    ...
    if (process.env.DEBUG_MEMORY) {
      const memUsage = process.memoryUsage();
      console.log(`Memory cleanup: RSS=${Math.round(memUsage.rss/1024/1024)}MB, Cache size=${Object.keys(this.cachedContent).length}`);
    }
  }
