...
class ASDClient {
  ...
  async init() {
    ...
    try {
      ...
      try {
        ...
        console.warn(chalk.yellow('⚠ Terminal initialization failed, using fallback mode'));
      ...
      if (error.message.includes('terminal') || error.message.includes('TTY')) {
        ...
        console.error(chalk.yellow('  • Ensure you are running in a proper terminal environment'));
        console.error(chalk.yellow('  • Try running in a different terminal application'));
        console.error(chalk.yellow('  • Check that your terminal supports the required features'));
      } else if (error.message.includes('specs') || error.message.includes('parse')) {
        console.error(chalk.yellow('\nSpec file parsing failed. Suggestions:'));
        console.error(chalk.yellow('  • Check that spec files have valid markdown syntax'));
        console.error(chalk.yellow('  • Verify file permissions in the docs/specs directory'));
        console.error(chalk.yellow('  • Run with --debug flag for detailed error information'));
      } else if (error.message.includes('workflow') || error.message.includes('validation')) {
        ...
        console.error(chalk.yellow('  • Try running npm install to refresh dependencies'));
      }
  ...
  async initializeFallbackTerminal() {
    ...
    try {
      ...
      console.warn(chalk.yellow(`⚠ Running in fallback terminal mode (${this.termWidth}x${this.termHeight})`));
      console.warn(chalk.yellow('   Some advanced features may be limited'));

    } catch (error) {
      throw new Error(`Fallback terminal initialization failed: ${error.message}`);
    }
  }
  ...
  async initializeTerminalSize() {
    ...
    if (this.termWidth < 60) {
      console.warn(chalk.yellow(`⚠ Terminal width (${this.termWidth}) is very small. Recommend at least 80 columns.`));
    }
    if (this.termHeight < 20) {
      console.warn(chalk.yellow(`⚠ Terminal height (${this.termHeight}) is very small. Recommend at least 24 rows.`));
    }
  ...
  setupMemoryMonitoring() {
    ...
    this.memoryMonitor.on('critical', (entry) => {
      console.error(`Critical memory usage: ${entry.rss}MB RSS - triggering cleanup`);
      this.performMemoryCleanup();
    });
  ...
  performMemoryCleanup() {
    ...
    if (global.gc && (cleanedUp > 10 || Date.now() - this.lastGC > 300000)) { // 5 minutes
      global.gc();
      this.lastGC = Date.now();
    }
    ...
    if (process.env.DEBUG_MEMORY && cleanedUp > 0) {
      ...
      console.log(
        ...
        )}MB, Cache size=${Object.keys(this.cachedContent).length}, Cleaned up ${cleanedUp} items`
      );
    }
