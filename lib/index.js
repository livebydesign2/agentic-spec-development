...
class ASDClient {
  ...
  async renderSidePanel() {
    ...
    if (this.sideView === 'overview') {
      ...
      await this.renderRoadmapContent(contentArea);
    }
  }
  ...
  async renderRoadmapContent(contentArea) {
    // Build complete roadmap content
    const roadmapContent = [];

    // Product header
    roadmapContent.push({ text: '🚀 ASD - ROADMAP', color: 'brightCyan' });
    roadmapContent.push({ text: '', color: null }); // Empty line
    roadmapContent.push({ text: 'Agentic Spec Development', color: 'white' });
    roadmapContent.push({ text: 'AI-first terminal tool for spec', color: 'gray' });
    roadmapContent.push({ text: 'development & project mgmt', color: 'gray' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    // Current Focus
    roadmapContent.push({ text: '🎯 CURRENT FOCUS', color: 'brightGreen' });
    roadmapContent.push({ text: 'Version: 0.1.0-alpha', color: 'white' });
    roadmapContent.push({ text: 'Phase: PHASE-1B Foundation', color: 'cyan' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    // MVP Foundation section
    roadmapContent.push({ text: '✅ MVP FOUNDATION', color: 'brightYellow' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    const foundationFeatures = [
      'FEAT-012: Context Injection',
      'FEAT-013: Task Router System',
      'FEAT-014: Workflow State Mgr',
      'FEAT-018: Advanced CLI Cmds',
      'FEAT-019: Validation Manager',
      'FEAT-020: Multi-format Data',
      'FEAT-021: Project Templates',
      'FEAT-022: Auto Documentation',
      'FEAT-023: Testing Framework',
      'FEAT-024: Performance Monitor',
      'FEAT-025: Memory Management',
      'FEAT-026: Cache Optimization',
    ];

    foundationFeatures.forEach((featureName) => {
      roadmapContent.push({ text: `  ✓ ${featureName}`, color: 'green' });
    });

    roadmapContent.push({ text: '', color: null }); // Empty line

    // NEXT section with more items
    roadmapContent.push({ text: '⏭️  NEXT: v1.1.0 - 3-Tier Strategy', color: 'brightBlue' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    const nextItems = [
      '🔥 P0: Error Handling (FEAT-030)',
      '🔥 P0: Security Audit (FEAT-031)',
      '⚡ P1: Quality Gates (FEAT-032)',
      '⚡ P1: Plugin System (FEAT-033)',
      '⚡ P1: API Integration (FEAT-034)',
      '📋 P2: Integration Tests (FEAT-035)',
      '📋 P2: CI/CD Pipeline (FEAT-036)',
      '📋 P2: Analytics Dashboard (FEAT-037)',
      '📋 P2: Documentation Portal (FEAT-038)',
    ];
    ...
    // Future phases
    roadmapContent.push({ text: '🚀 FUTURE: v2.0 - Enterprise', color: 'brightMagenta' });
    roadmapContent.push({ text: '', color: null }); // Empty line
    roadmapContent.push({ text: '  • Multi-user collaboration', color: 'magenta' });
    roadmapContent.push({ text: '  • Cloud sync & backup', color: 'magenta' });
    roadmapContent.push({ text: '  • Advanced AI models', color: 'magenta' });
    roadmapContent.push({ text: '  • Custom workflow builder', color: 'magenta' });
    roadmapContent.push({ text: '  • Enterprise SSO', color: 'magenta' });
    roadmapContent.push({ text: '  • Audit logging', color: 'magenta' });
    roadmapContent.push({ text: '  • Role-based access', color: 'magenta' });
    ...
    visibleContent.forEach((line) => {
      if (currentY < contentArea.height) {
        ...
        if (line.color && term[line.color]) {
          term[line.color](line.text.substring(0, contentArea.width - 2)); // Leave space for scrollbar
        } else {
          term(line.text.substring(0, contentArea.width - 2));
        }
      }
    });

    // Draw scroll bar if content exceeds visible area
    if (roadmapContent.length > maxVisibleLines) {
      this.drawRoadmapScrollBar(contentArea, scrollOffset, roadmapContent.length, maxVisibleLines);
    }
  }

  drawRoadmapScrollBar(contentArea, scrollOffset, totalLines, visibleLines) {
    const scrollBarX = contentArea.x + contentArea.width - 1;
    const scrollBarHeight = contentArea.height;
    ...
    const thumbPosition = Math.floor((scrollOffset / (totalLines - visibleLines)) * maxThumbPosition);

    // Draw scroll track
    for (let i = 0; i < scrollBarHeight; i++) {
      term.moveTo(scrollBarX, contentArea.y + i);
    ...
    // Draw scroll indicators at top and bottom
    if (scrollOffset > 0) {
      term.moveTo(scrollBarX, contentArea.y);
      term.brightWhite('↑');
    }

    if (scrollOffset + visibleLines < totalLines) {
      term.moveTo(scrollBarX, contentArea.y + scrollBarHeight - 1);
      term.brightWhite('↓');
    }
  }
