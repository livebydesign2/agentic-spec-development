...
class ASDClient {
  constructor(options = {}) {
    ...
    // Side panel view state for Overview/Roadmap tabs
    this.sideView = 'overview'; // 'overview' or 'roadmap'
    ...
    // Memory management - Enhanced with monitoring
    this.renderCount = 0;
    this.lastGC = Date.now();
    this.memoryCleanupInterval = null;
    this.maxCacheSize = this.config.maxCacheSize || 100; // Limit cached content entries

    // Initialize memory monitor
    this.memoryMonitor = new MemoryMonitor({
      warningThreshold: this.config.memoryWarningThreshold || 150,
      criticalThreshold: this.config.memoryCriticalThreshold || 200,
      monitorInterval: this.config.memoryMonitorInterval || 10000, // 10 seconds
      enableLogging: this.config.enableMemoryLogging !== false,
      autoCleanup: this.config.autoMemoryCleanup !== false,
    });
    ...
    this.scrollOffsets = {
      ...
      side: 0,
      bottom: 0,
      roadmap: 0,  // Add roadmap scroll offset
      recommended: 0,
      overview: 0,
    };

    // Track total content lines for scroll calculation
  ...
  async init() {
    ...
    try {
      ...
      console.log(chalk.green('✅ Validation manager initialized'));

      // Setup terminal-kit with enhanced error handling and fallbacks
      try {
        await this.initializeTerminal();
      } catch (terminalError) {
        console.warn(
          chalk.yellow('⚠ Terminal initialization failed, using fallback mode')
        );
        console.warn(chalk.yellow(`   ${terminalError.message}`));
      ...
      this.setupMemoryMonitoring(); // Start memory monitoring
      ...
      // Provide contextual error guidance
      if (error.message.includes('terminal') || error.message.includes('TTY')) {
        console.error(
          chalk.yellow('\nTerminal initialization failed. Suggestions:')
        );
        console.error(
          chalk.yellow(
            '  • Ensure you are running in a proper terminal environment'
          )
        );
        console.error(
          chalk.yellow('  • Try running in a different terminal application')
        );
        console.error(
          chalk.yellow(
        ...
        error.message.includes('specs') ||
        error.message.includes('parse')
      ) {
        console.error(chalk.yellow('\nSpec file parsing failed. Suggestions:'));
        console.error(
          chalk.yellow('  • Check that spec files have valid markdown syntax')
        );
        console.error(
          chalk.yellow(
            '  • Verify file permissions in the docs/specs directory'
          )
        );
        console.error(
          chalk.yellow(
        ...
        console.error(
          chalk.yellow('\nSystem initialization failed. Suggestions:')
        );
        console.error(chalk.yellow('  • Check file system permissions'));
        console.error(chalk.yellow('  • Verify project structure is valid'));
        console.error(
          chalk.yellow('  • Try running npm install to refresh dependencies')
        );
      }
  ...
  async initializeTerminal() {
    // Full terminal initialization with error handling
    try {
      term.clear();
      ...
      if (process.env.DEBUG_STARTUP) {
        console.log(chalk.green('✅ Terminal initialized successfully'));
      }
    } catch (error) {
  ...
  async initializeFallbackTerminal() {
    // Minimal terminal setup for fallback mode
    try {
      // Don't use fullscreen or advanced features in fallback mode
      term.clear();

      // Just set basic terminal size using process.stdout
      this.termWidth = process.stdout.columns || 80;
      this.termHeight = process.stdout.rows || 24;

      // Override term properties with fallback values
      ...
      console.warn(
        chalk.yellow(
          `⚠ Running in fallback terminal mode (${this.termWidth}x${this.termHeight})`
        )
      );
      console.warn(chalk.yellow('   Some advanced features may be limited'));
    } catch (error) {
      throw new Error(
        `Fallback terminal initialization failed: ${error.message}`
      );
    }
  }
  ...
  async initializeTerminalSize() {
    ...
    if (
      ...
      if (process.env.DEBUG_STARTUP) {
        console.warn(chalk.yellow('⚠ Using fallback terminal dimensions'));
      }
    } else {
      this.termWidth = term.width;
      this.termHeight = term.height;
    ...
    if (this.termWidth < 60) {
      console.warn(
        chalk.yellow(
          `⚠ Terminal width (${this.termWidth}) is very small. Recommend at least 80 columns.`
        )
      );
    }
    if (this.termHeight < 20) {
      console.warn(
        chalk.yellow(
          `⚠ Terminal height (${this.termHeight}) is very small. Recommend at least 24 rows.`
        )
      );
    }
  ...
  drawPanelBorders(layout) {
    ...
    const sideTabbedHeader = this.ui.createSideTabbedHeader(
      this.sideView,
      this.focusedPanel,
      panelCoords.side.width - 2
    );
    const sideColor =
  ...
  setupKeyBindings() {
    term.on('key', (name, _matches, _data) => {
      ...
      // Page up/down for roadmap scrolling
      if (this.focusedPanel === 'side' && this.sideView === 'roadmap') {
        if (name === 'PAGE_UP') {
          const layout = this.calculateLayout();
          const panelCoords = this.calculatePanelBorderCoordinates(layout);
          const contentArea = this.ui.createContentArea(panelCoords.side, 2);
          const pageSize = Math.max(1, contentArea.height - 2);
          this.scrollOffsets.roadmap = Math.max(0, this.scrollOffsets.roadmap - pageSize);
          this.render();
        }
        ...
        if (name === 'PAGE_DOWN') {
          ...
          const contentArea = this.ui.createContentArea(panelCoords.side, 2);
          const pageSize = Math.max(1, contentArea.height - 2);
          const maxScroll = Math.max(0, this.contentLineCount.roadmap - contentArea.height);
          this.scrollOffsets.roadmap = Math.min(maxScroll, this.scrollOffsets.roadmap + pageSize);
          this.render();
        }

        if (name === 'HOME') {
          this.scrollOffsets.roadmap = 0;
          this.render();
        }
        ...
        if (name === 'END') {
          ...
          const contentArea = this.ui.createContentArea(panelCoords.side, 2);
          const maxScroll = Math.max(0, this.contentLineCount.roadmap - contentArea.height);
          this.scrollOffsets.roadmap = maxScroll;
          this.render();
        }
      }
    });

    // Mouse wheel support for roadmap scrolling
    term.on('mouse', (name, data) => {
      if (this.focusedPanel === 'side' && this.sideView === 'roadmap') {
        const layout = this.calculateLayout();
        const panelCoords = this.calculatePanelBorderCoordinates(layout);

        // Check if mouse is within side panel
        if (data.x >= panelCoords.side.x &&
            data.x < panelCoords.side.x + panelCoords.side.width &&
            data.y >= panelCoords.side.y &&
            data.y < panelCoords.side.y + panelCoords.side.height) {

          if (name === 'WHEEL_UP') {
            this.scrollOffsets.roadmap = Math.max(0, this.scrollOffsets.roadmap - 3);
            this.render();
          } else if (name === 'WHEEL_DOWN') {
            const contentArea = this.ui.createContentArea(panelCoords.side, 2);
            const maxScroll = Math.max(0, this.contentLineCount.roadmap - contentArea.height);
            this.scrollOffsets.roadmap = Math.min(maxScroll, this.scrollOffsets.roadmap + 3);
            this.render();
          }
        }
      }
  ...
  navigateUp() {
    ...
    } else if (this.focusedPanel === 'side' && this.sideView === 'roadmap') {
      // Scroll up in roadmap
      this.scrollOffsets.roadmap = Math.max(0, this.scrollOffsets.roadmap - 1);
    }
  }
  ...
  navigateDown() {
    if (this.focusedPanel === 'main') {
      ...
      // Scroll down in roadmap
      const layout = this.calculateLayout();
      const panelCoords = this.calculatePanelBorderCoordinates(layout);
      const contentArea = this.ui.createContentArea(panelCoords.side, 2);
      const maxScroll = Math.max(0, this.contentLineCount.roadmap - contentArea.height);
      this.scrollOffsets.roadmap = Math.min(maxScroll, this.scrollOffsets.roadmap + 1);
    }
  }
  ...
  switchView(direction) {
    ...
    } else if (this.focusedPanel === 'side') {
      const views = ['overview', 'roadmap'];
      const currentIndex = views.indexOf(this.sideView);
      this.sideView = views[(currentIndex + 1) % views.length];
    }
  }
  ...
  async renderMainPanel() {
    ...
    // Store content line count for main panel
    this.contentLineCount.main = features.length;

    // Draw scroll bar using universal function
    this.drawScrollBar(panelCoords.main, this.scrollOffsets.main, features.length, maxVisibleFeatures, 'main');
  }
  ...
  async renderSidePanel() {
    ...
    if (this.sideView === 'overview') {
      await this.renderOverviewContent(contentArea);
    } else {
      await this.renderRoadmapContent(contentArea, panelCoords.side);
    }
  }

  async renderOverviewContent(contentArea) {
  ...
  async renderRoadmapContent(contentArea, panelCoords) {
    // Build complete roadmap content
    const roadmapContent = [];

    // Product header
    roadmapContent.push({ text: '🚀 MVP ROADMAP', color: 'brightCyan' });
    roadmapContent.push({ text: '', color: null }); // Empty line
    roadmapContent.push({ text: 'Ready for Smoke Testing', color: 'white' });
    roadmapContent.push({ text: 'Focus: Clean UI, Fast Performance', color: 'gray' });
    roadmapContent.push({ text: 'Timeline: 1-2 weeks to MVP', color: 'gray' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    // Current Focus
    roadmapContent.push({ text: '🎯 CURRENT FOCUS', color: 'brightGreen' });
    roadmapContent.push({ text: 'Version: 0.1.0-alpha', color: 'white' });
    roadmapContent.push({ text: 'Phase: MVP-CRITICAL', color: 'brightYellow' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    // MVP Critical Path
    roadmapContent.push({ text: '🔴 MVP CRITICAL PATH', color: 'brightRed' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    const mvpCritical = [
      '🔥 FEAT-035: UI Improvements [P0]',
      '',
      '🔥 MAINT-005: Performance [P0]',
      '',
      '⚡ FEAT-030: Error Handling [P1]',
    ];

    mvpCritical.forEach((item) => {
      if (item === '') {
        roadmapContent.push({ text: '', color: null });
      } else if (item.startsWith('🔥')) {
        roadmapContent.push({ text: item, color: 'brightYellow' });
      } else if (item.startsWith('⚡')) {
        roadmapContent.push({ text: item, color: 'cyan' });
      } else {
        roadmapContent.push({ text: item, color: 'gray' });
      }
    });
    ...
    // Completed section
    roadmapContent.push({ text: '✅ COMPLETED FOUNDATION', color: 'brightGreen' });
    roadmapContent.push({ text: '', color: null }); // Empty line

    const completed = [
      'FEAT-012: Context Injection',
      'FEAT-013: Task Router',
      'FEAT-014: Workflow Manager',
      'MAINT-001-004: Core Stability',
      'BUG-003-004: Critical Fixes',
    ];

    completed.forEach((item) => {
      roadmapContent.push({ text: `  ✓ ${item}`, color: 'green' });
    });

    roadmapContent.push({ text: '', color: null }); // Empty line

    // Post-MVP section
    roadmapContent.push({ text: '🚀 POST-MVP (Backlog)', color: 'brightMagenta' });
    roadmapContent.push({ text: '', color: null }); // Empty line
    roadmapContent.push({ text: '  • NPM package publishing', color: 'magenta' });
    roadmapContent.push({ text: '  • Pre-commit quality gates', color: 'magenta' });
    roadmapContent.push({ text: '  • Integration testing', color: 'magenta' });
    roadmapContent.push({ text: '  • Plugin architecture', color: 'magenta' });
    roadmapContent.push({ text: '  • Theme customization', color: 'magenta' });
    roadmapContent.push({ text: '  • Multi-user support', color: 'magenta' });
    roadmapContent.push({ text: '  • Cloud sync', color: 'magenta' });

    // Store total line count
    this.contentLineCount.roadmap = roadmapContent.length;

    // Calculate scroll position
    const maxVisibleLines = contentArea.height;
    let scrollOffset = this.scrollOffsets.roadmap || 0;

    // Adjust scroll offset to keep it within bounds
    scrollOffset = Math.max(0, Math.min(scrollOffset, Math.max(0, roadmapContent.length - maxVisibleLines)));
    this.scrollOffsets.roadmap = scrollOffset;

    // Render visible content
    const visibleContent = roadmapContent.slice(scrollOffset, scrollOffset + maxVisibleLines);

    let currentY = 0;
    visibleContent.forEach((line) => {
      if (currentY < contentArea.height) {
        term.moveTo(contentArea.x, contentArea.y + currentY++);
        if (line.color && term[line.color]) {
          // Leave space for scrollbar when it's visible (1 column inside border)
          const maxWidth = (this.focusedPanel === 'side' && roadmapContent.length > maxVisibleLines)
            ? contentArea.width - 1
            : contentArea.width;
          term[line.color](line.text.substring(0, maxWidth));
        } else {
          const maxWidth = (this.focusedPanel === 'side' && roadmapContent.length > maxVisibleLines)
            ? contentArea.width - 1
            : contentArea.width;
          term(line.text.substring(0, maxWidth));
        }
      }
    });

    // Draw scroll bar using universal function
    this.drawScrollBar(panelCoords || contentArea, scrollOffset, roadmapContent.length, maxVisibleLines, 'side');
  }

  // Universal scroll bar drawing function for all panels
  drawScrollBar(panelCoords, scrollOffset, totalLines, visibleLines, panelName) {
    // Don't draw if content fits or panel not focused
    if (totalLines <= visibleLines || this.focusedPanel !== panelName) {
      return;
    }

    // Position scroll bar at x + width - 1 (centered on the border line)
    const scrollBarX = panelCoords.x + panelCoords.width - 1;
    // Full height inside the borders (from row after top border to row before bottom border)
    const scrollBarHeight = panelCoords.height - 2; // Subtract top and bottom border rows
    const scrollBarY = panelCoords.y + 1; // Start at row after top border

    // Calculate thumb position and size
    const thumbSize = Math.max(1, Math.floor((visibleLines / totalLines) * scrollBarHeight));
    const maxThumbPosition = scrollBarHeight - thumbSize;
    const thumbPosition = Math.floor((scrollOffset / Math.max(1, totalLines - visibleLines)) * maxThumbPosition);

    // Draw full-height scroll bar with light green track and green thumb
    for (let i = 0; i < scrollBarHeight; i++) {
      term.moveTo(scrollBarX, scrollBarY + i);

      // Check if this is the thumb position
      if (i >= thumbPosition && i < thumbPosition + thumbSize) {
        // Green thumb matching focus outline when panel is focused
        term.brightGreen('▓');  // Darker shade block for better contrast
      } else {
        // Light green track showing full scrollable area
        term.color(82)('░'); // Light green block using 256-color mode
      }
    }
  }
  ...
  setupFileWatching() {
    ...
    try {
      ...
      const watcher = chokidar.watch(path.join(dataPath, '**/*.md'), {
        ...
        // Limit memory usage with chokidar options
        usePolling: false, // Use native file system events when possible
        atomic: true, // Helps prevent duplicate events
      });

      // Use bound methods with proper cleanup to prevent memory leaks
      const boundRefresh = (filePath) => {
        // Invalidate cache for changed file
        if (this.specParser) {
          this.specParser.invalidateFileCache(filePath);
        }
      ...
      const eventHandlers = {
        ...
        error: (error) => {
          ...
          // Don't let file watcher errors crash the app
        },
      };
      ...
      // Store watcher with comprehensive cleanup info
      this.watchers.push({
        watcher: watcher,
        ...
        cleanup: () => {
          try {
            ...
            if (process.env.DEBUG_MEMORY) {
              console.warn('Watcher cleanup error:', error.message);
            }
          }
        },
        createdAt: Date.now(),
      ...
      console.log(`File watching enabled for: ${dataPath}`);
    } catch (error) {
  ...
  setupMemoryMonitoring() {
    ...
    this.memoryMonitor.on('warning', (entry) => {
      if (process.env.DEBUG_MEMORY) {
        console.warn(`Memory warning: ${entry.rss}MB RSS usage`);
      }
    });

    this.memoryMonitor.on('critical', (entry) => {
      console.error(
        `Critical memory usage: ${entry.rss}MB RSS - triggering cleanup`
      );
      this.performMemoryCleanup();
    });

    this.memoryMonitor.on('leak-detected', (leakInfo) => {
      console.error('Memory leak detected:', leakInfo);
      // Force aggressive cleanup
      this.performMemoryCleanup();
      this.specParser?.performCacheMaintenance();
    });
  ...
  performMemoryCleanup() {
    ...
    if (this.renderQueue.length > 10) {
      const removedItems = this.renderQueue.length - 5;
      this.renderQueue = this.renderQueue.slice(-5); // Keep only last 5 items
      cleanedUp += removedItems;
    }

    // Clean up old file modification times if they exist
    if (this.fileMTimes && this.fileMTimes.size > this.maxCacheSize * 2) {
      const oldEntries = Array.from(this.fileMTimes.entries())
        .sort(([, a], [, b]) => a - b)
        .slice(0, this.fileMTimes.size - this.maxCacheSize);

      oldEntries.forEach(([filePath]) => {
        this.fileMTimes.delete(filePath);
        cleanedUp++;
      });
    }

    // Force garbage collection if available and we cleaned up significant items
    if (global.gc && (cleanedUp > 10 || Date.now() - this.lastGC > 300000)) {
      // 5 minutes
      global.gc();
      this.lastGC = Date.now();
    }
    ...
    if (process.env.DEBUG_MEMORY && cleanedUp > 0) {
      ...
      console.log(
        `Memory cleanup: RSS=${Math.round(
          ...
          Object.keys(this.cachedContent).length
        }, Cleaned up ${cleanedUp} items`
      );
    }
