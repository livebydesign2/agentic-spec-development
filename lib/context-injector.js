...
class ContextInjector {
  ...
  async injectContextForTask(options) {
    ...
    try {
      ...
      if (!agentType || !specId || !taskId) {
        throw new Error(
          'agentType, specId, and taskId are required for automated task context injection'
        );
      }
      ...
      throw new Error(
        `Task context injection failed after ${totalTime}ms: ${error.message}`
      );
    }
  }
  ...
  async performAutomatedContextInjection(options) {
    ...
    mergedContext.automation.validation = await this.validateAutomatedContext(
      mergedContext,
      agentType
    );
    ...
    if (totalTime > 3000) {
      console.warn(
        `Automated context injection took ${totalTime}ms, exceeding 3000ms target for automation workflows`
      );
    }
  ...
  async validateAutomatedContext(context, _agentType) {
    ...
    if (
      !taskContext.taskSpecific.task ||
      !taskContext.taskSpecific.task.checklist
    ) {
    ...
    if (taskContext.validation.relevanceScore >= 0.6) {
      ...
      validation.issues.push(
        `Low relevance score: ${taskContext.validation.relevanceScore}`
      );
    }
    ...
    validation.isReady =
      validation.score >= 0.7 && validation.issues.length === 0;

    // Add recommendations for improvement
    if (!validation.isReady) {
      validation.recommendations.push(
        'Consider manual context preparation or task specification review'
      );
    }
  ...
  async getContextualTaskRecommendations(agentType, preferences = {}) {
    try {
      ...
      const recommendations = await taskRouter.getNextTask(
        agentType,
        preferences
      );
        ...
        try {
          ...
          contextualRecommendations.push({
            ...
            contextPreview: {
              ...
              estimatedContextTime:
                contextPreview.metadata.performance.gatheringTimeMs,
              hasRequiredContext: contextPreview.validation.isSufficient,
            },
          });
        } catch (error) {
          console.warn(
            `Failed to gather context preview for ${rec.specId}-${rec.taskId}: ${error.message}`
          );
          contextualRecommendations.push({
            ...rec,
            contextPreview: {
      ...
      contextualRecommendations.sort((a, b) => {
        const scoreA =
          a.contextPreview.relevanceScore * 0.7 +
          a.contextPreview.completeness * 0.3;
        const scoreB =
          b.contextPreview.relevanceScore * 0.7 +
          b.contextPreview.completeness * 0.3;
      ...
      throw new Error(
        `Failed to get contextual task recommendations: ${error.message}`
      );
    }
  }
