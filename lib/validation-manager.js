...
class ValidationManager {
  ...
  async initialize() {
    ...
    try {
      ...
      this.performanceThreshold =
        this.validationConfig.performanceThreshold || 2000;
      this.autoFixingEnabled = this.validationConfig.autoFixing !== false;
      this.confirmationRequired =
        this.validationConfig.requireConfirmation !== false;
      ...
      throw new Error(
        `ValidationManager initialization failed: ${error.message}`
      );
    }
  }
  ...
  async validateProject() {
    ...
    try {
      ...
      for (const spec of specs) {
        const specResult = await this.validateSpec(spec.filePath, {
          skipCache: true,
        });
    ...
    try {
      ...
      if (spec.tasks && Array.isArray(spec.tasks)) {
        for (const task of spec.tasks) {
          const taskResult = await this.validateTask(spec.id, task.id, {
            task,
            spec,
          });
  ...
  registerRule(name, rule) {
    try {
      ...
      if (!rule.category || !this.ruleCategories.includes(rule.category)) {
        throw new Error(
          `Rule must have a valid category: ${this.ruleCategories.join(', ')}`
        );
      }
      ...
      console.warn(
        `Failed to register validation rule ${name}: ${error.message}`
      );
      return false;
    }
  }
  ...
  async autoFix(validationResults, options = {}) {
    ...
    try {
      ...
      if (
        ...
        validationResults.fixable.length === 0
      ) {
        return {
          ...results,
    ...
    try {
      ...
      switch (error.type) {
        case 'missing_required_field':
          suggestions.push(
            `Add the missing field '${error.field}' to the frontmatter`
          );
          break;
        case 'invalid_id_format':
          suggestions.push(
            `Update ID to follow pattern: ${error.expectedPattern}`
          );
          break;
  ...
  async previewFixes(validationResults) {
    ...
    try {
      ...
      for (const issue of validationResults.fixable) {
        ...
        if (
          rule &&
          typeof rule.canAutoFix === 'function' &&
          rule.canAutoFix(issue)
        ) {
  ...
  async enforceQualityGate(operation, data) {
    try {
      const validationResult = await this.validateWorkflowOperation(
        operation,
        data
      );
  ...
  _initializeBuiltInRules() {
    ...
    this.registerRule(
      'required-fields',
      new ValidationRules.RequiredFieldsRule()
    );
    this.registerRule('id-format', new ValidationRules.IDFormatRule());
    this.registerRule(
      'priority-validation',
      new ValidationRules.PriorityValidationRule()
    );
    this.registerRule(
      'status-validation',
      new ValidationRules.StatusValidationRule()
    );

    // Register task validation rules
    this.registerRule(
      'task-dependencies',
      new ValidationRules.TaskDependencyRule()
    );
    this.registerRule(
      'agent-type-validation',
      new ValidationRules.AgentTypeValidationRule()
    );
    this.registerRule(
      'task-structure',
      new ValidationRules.TaskStructureRule()
    );
    ...
    this.registerRule(
      ...
      new ValidationRules.DependencyValidationRule()
    );

    // Register MAINT-003 TASK-004 validation rules
    this.registerRule(
      'reference-integrity',
      new ValidationRules.ReferenceIntegrityRule()
    );

    // Register workflow validation rules
    this.registerRule(
      'assignment-validation',
      new ValidationRules.AssignmentValidationRule()
    );
    this.registerRule(
      'transition-validation',
      new ValidationRules.TransitionValidationRule()
    );
  }
