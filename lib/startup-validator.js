...
class StartupValidator {
  ...
  async validateEnvironment() {
    ...
    try {
      ...
      results.metrics.phases.terminalValidation =
        Date.now() - terminalPhaseStart;
      ...
      results.metrics.phases.permissionsValidation =
        Date.now() - permissionsPhaseStart;
      ...
      results.metrics.phases.terminalSizeValidation =
        Date.now() - terminalSizePhaseStart;
      ...
      results.metrics.phases.cliEnvironmentValidation =
        Date.now() - cliEnvPhaseStart;
  ...
  validateNodeVersion() {
    ...
    try {
      ...
      result.valid = false;
      result.errors.push(
        `Failed to validate Node.js version: ${error.message}`
      );
    }
  ...
  async validateTerminalCapabilities() {
    ...
    try {
      ...
      if (!process.stdout.isTTY) {
        result.warnings.push(
          'Not running in a TTY environment. Some features may be limited.'
        );
      }
      ...
      if (!process.stdout.hasColors || !process.stdout.hasColors()) {
        result.warnings.push(
          'Terminal does not support colors. Output will be monochrome.'
        );
      }
      ...
      if (termType === 'unknown') {
        result.warnings.push(
          'Terminal type unknown. Some features may not work correctly.'
        );
      } else if (this.debug) {
      ...
      result.warnings.push(
        `Terminal capability detection failed: ${error.message}`
      );
    }
  ...
  async validateDependencies() {
    ...
    for (const dep of criticalDependencies) {
      try {
        ...
        result.valid = false;
        result.errors.push(
          `Critical dependency '${dep}' not found or not loadable`
        );
    ...
    for (const dep of optionalDependencies) {
      try {
        ...
        result.warnings.push(
          `Optional dependency '${dep}' not found. Some features may be limited.`
        );
      }
    }
  ...
  validateProjectStructure() {
    ...
    try {
      ...
      if (!hasAnyStructure) {
        ...
        result.warnings.push('ASD will create missing directories as needed.');
      }
      ...
      if (fs.existsSync(configPath)) {
        try {
          ...
          result.errors.push(
            `Configuration file exists but has syntax errors: ${error.message}`
          );
          result.errors.push(
      ...
      result.warnings.push(
        `Project structure validation failed: ${error.message}`
      );
    }
  ...
  async validateFileSystemPermissions() {
    ...
    try {
      ...
      try {
        ...
        result.warnings.push(
          `Limited write access to current directory: ${this.cwd}`
        );
        result.warnings.push(
          'You may not be able to create new specifications'
        );
      }
      ...
      for (const testPath of testPaths) {
        ...
        if (fs.existsSync(fullPath)) {
          try {
            ...
            result.warnings.push(
              `Limited access to ${testPath}. Some features may not work.`
            );
          }
        }
      }
      ...
      result.warnings.push(
        `File system permissions check failed: ${error.message}`
      );
    }
  ...
  displayResults(results) {
    ...
    if (results.valid) {
      if (this.debug && results.metrics.validationTime) {
        console.log(
          chalk.green(
            `✓ Environment validation passed (${results.metrics.validationTime}ms)`
          )
        );
      }
    } else {
  ...
  generateErrorSuggestions(results) {
    ...
    if (!results.valid) {
      ...
      // Permission issues
      if (
        results.errors.some((err) => err.includes('access')) ||
  ...
  logStartupAttempt(results) {
    ...
    this.writeStartupLog(logEntry).catch((error) => {
      ...
      if (this.debug) {
        console.warn(
          chalk.yellow(`Warning: Failed to write startup log: ${error.message}`)
        );
      }
    });
  }
  ...
  validateTerminalSize() {
    ...
    if (width < minWidth) {
      ...
      result.errors.push(
        'Please resize your terminal window or use a larger display'
      );
      result.valid = false;
  ...
  validateCLIEnvironment() {
    ...
    try {
      ...
      try {
        ...
        proc.on('error', () => {
          result.warnings.push(
            'ASD not found in PATH. Consider running: npm link (for development)'
          );
        });
      ...
      devTools.forEach((tool) => {
        try {
          ...
          if (tool === 'git') {
            result.warnings.push(
              'Git not found. Some ASD features may be limited.'
            );
          }
        }
      });
  ...
  analyzePerformance(metrics) {
    ...
    if (totalTime > SLOW_TOTAL_THRESHOLD) {
      ...
      analysis.recommendations.push(
        'Startup validation is slower than expected'
      );
    }
    ...
    if (analysis.slowPhases.some((p) => p.phase.includes('dependency'))) {
      analysis.recommendations.push(
        'Consider using npm ci instead of npm install for faster dependency resolution'
      );
    }

    if (analysis.slowPhases.some((p) => p.phase.includes('permissions'))) {
      analysis.recommendations.push(
        'File system permissions check is slow - check disk performance or antivirus interference'
      );
    }

    if (analysis.slowPhases.some((p) => p.phase.includes('project'))) {
      analysis.recommendations.push(
        'Project structure scan is slow - consider organizing spec files more efficiently'
      );
    }
  ...
  generateStartupReport(results) {
    const report = {
      ...
      environment: {
        ...
        platform: process.platform,
        cwd: this.cwd,
        terminalSize: `${process.stdout.columns || 80}x${
