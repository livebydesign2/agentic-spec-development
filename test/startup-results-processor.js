...
module.exports = (testResults) => {
  ...
  startupMetrics.performanceMetrics = calculateOverallMetrics(startupMetrics.testSuites);

  // Check for performance regressions
  startupMetrics.regressionAlerts = checkForRegressions(startupMetrics.performanceMetrics);
...
function calculateOverallMetrics(testSuites) {
  ...
  allPerformanceTests.forEach(test => {
    test.metrics.forEach(metric => {
      ...
      metrics[metric.operation].min = Math.min(metrics[metric.operation].min, metric.duration);
      metrics[metric.operation].max = Math.max(metrics[metric.operation].max, metric.duration);
    });
  });
  ...
  Object.keys(metrics).forEach(operation => {
    ...
    metrics[operation].average = samples.reduce((sum, val) => sum + val, 0) / samples.length;
...
function checkForRegressions(performanceMetrics) {
  ...
  Object.keys(performanceMetrics).forEach(operation => {
    ...
    if (metric.sampleCount > 3) {
      ...
      if (coefficient > 0.3) { // More than 30% coefficient of variation
        alerts.push({
...
function generatePerformanceReport(metrics) {
  try {
    ...
    if (Object.keys(metrics.performanceMetrics).length > 0) {
      ...
      report += `| Operation | Average (ms) | Min (ms) | Max (ms) | Samples |\n`;
      report += `|-----------|--------------|----------|----------|---------|\n`;
      
      Object.entries(metrics.performanceMetrics).forEach(([operation, data]) => {
        report += `| ${operation} | ${data.average.toFixed(2)} | ${data.min.toFixed(2)} | ${data.max.toFixed(2)} | ${data.sampleCount} |\n`;
      });
      report += `\n`;
    }
    ...
    if (metrics.regressionAlerts.length > 0) {
      ...
      metrics.regressionAlerts.forEach(alert => {
        ...
        report += `${severity} **${alert.type.replace('_', ' ').toUpperCase()}** - ${alert.operation}\n`;
        
        if (alert.average && alert.threshold) {
          report += `  - Average: ${alert.average.toFixed(2)}ms (threshold: ${alert.threshold}ms)\n`;
        }
        if (alert.coefficient) {
          report += `  - Performance variance: ${(alert.coefficient * 100).toFixed(1)}%\n`;
        }
        report += `\n`;
      });
    } else {
