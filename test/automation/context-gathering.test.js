...
`;
        ...
        expect(context.metadata).to.have.property('agentType', 'software-architect');
...
- **Agent**: software-architect`;
      ...
      it('should handle missing specifications gracefully', async function() {
        ...
        try {
          ...
          expect(error.message).to.include('Specification NONEXISTENT not found');
        }
      });
      ...
      it('should complete context gathering within performance targets', async function() {
        ...
        expect(context.metadata.performance.gatheringTimeMs).to.be.lessThan(5000);
      });
    });
...
Dependency content`;
        ...
        sandbox.stub(fs, 'readFile')
          .onFirstCall().resolves(mockSpec)
          .onSecondCall().resolves(mockDepSpec);
        sandbox.stub(fs, 'access').resolves();
        sandbox.stub(contextGatherer, 'findAllSpecifications').resolves([]);
  ...
  describe('Enhanced ContextInjector', function() {
    describe('injectContextForTask()', function() {
      ...
      it('should meet automation performance targets', async function() {
        ...
        expect(context.metadata.performance.automatedInjection).to.be.lessThan(3000);
      });
    });
    ...
    describe('getContextualTaskRecommendations()', function() {
      it('should provide task recommendations with context relevance', async function() {
        ...
        const recommendations = await contextInjector.getContextualTaskRecommendations('software-architect');

        expect(recommendations.recommendations).to.have.length(2);
        expect(recommendations.recommendations[0]).to.have.property('contextPreview');
        expect(recommendations.recommendations[0].contextPreview.relevanceScore).to.equal(0.8);
        expect(recommendations.recommendations[0].contextPreview.hasRequiredContext).to.be.true;
      });
      ...
      it('should sort recommendations by relevance score', async function() {
        ...
        sandbox.stub(contextInjector.contextGatherer, 'gatherTaskContext')
          ...
          .onSecondCall().resolves({
            validation: { relevanceScore: 0.9, completeness: 0.8, isSufficient: true },
  ...
  describe('Integration Tests', function() {
    ...
    it('should maintain performance under load', async function() {
      ...
      for (let i = 0; i < 10; i++) {
        promises.push(contextGatherer.gatherTaskContext({
          specId: `FEAT-${i}`,
          taskId: 'TASK-001',
          agentType: 'software-architect'
        }));
      ...
      results.forEach(result => {
        expect(result.metadata.performance.gatheringTimeMs).to.be.lessThan(5000);
      });
    });
  });
});